...
struct DevicePointers {
	...
	DevicePointers(device_real_t* X, device_real_t* X_temp, device_real_t* X2, device_node_t* neighbours, device_node_t* next_on_face, device_node_t* prev_on_face, uint8_t* face_right, device_real_t* gdata) : 
        X(X), X1(X1), X2(X2), neighbours(neighbours), next_on_face(next_on_face), prev_on_face(prev_on_face), face_right(face_right), gdata(gdata) {}
...
struct HostPointers
{
	...
	HostPointers(device_real_t* h_X,
		       ...
		       device_node_t* h_next_on_face,
		       device_node_t* h_prev_on_face,
		       uint8_t* h_face_right) : h_X(h_X), h_cubic_neighbours(h_cubic_neighbours), h_next_on_face(h_next_on_face), h_prev_on_face(h_prev_on_face), h_face_right(h_face_right) {}
};
