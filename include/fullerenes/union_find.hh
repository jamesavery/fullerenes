...
public:
  ...
  // Partition {0..n-1} into components. Members are listed in ascending order;
  // components appear in order of their smallest member.
...
// Orbit partition of {0..n-1} under the group generated by the permutations G:
// the connected components of the relation u ~ pi[u]. Each pi need only be
// indexable as pi[u] for u in [0,n), so this serves both vector<Permutation>
// and vector<vector<int>> callers. O(n*|G|*alpha(n)) time, O(n) space.
template<class Perm>
