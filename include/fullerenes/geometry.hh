...
// Work counters for convex_hull_tris (optional out-parameter): machine-
// independent evidence of the construction's cost, so a complexity regression
// is testable as a COUNT bound instead of a flaky wall-clock bound. On an
// n-point input the conflict-graph construction does expected O(n log n)
// conflict tests; an accidental return to the quadratic scan shows up as
// conflict_tests ~ n^2.
struct HullStats {
  size_t faces_created  = 0;  // fan faces built over all insertions
  size_t conflict_tests = 0;  // point-vs-face plane evaluations (init + BFS + redistribution)
  size_t points_inserted = 0; // points that became hull vertices at some stage
};

// Convex hull of a 3D point set as a triangle list (indices into pts), outward-
// oriented normals. Conflict-graph incremental construction (Clarkson-Shor
// style): maintained twin adjacency, per-face cached planes, and per-point
// conflict witnesses -- expected O(n log n) point-face tests over a fixed-seed
// deterministic shuffle of the insertion order (same input => same output; no
// joggle). eps-APPROXIMATE hull with eps = 1e-10 * (bbox diagonal): points
// within eps of the hull surface may be omitted from the output vertex set (so
// exactly-coplanar or on-edge subdivision points do not appear as spurious
// vertices). Guarantees on the result: watertight (every directed edge appears
// exactly once, its twin exactly once -- audited in a final O(F) pass),
// outward-oriented, and every input point lies <= O(eps) outside every output
// face plane. Degenerate input -- fewer than 4 points, or points not spanning
// 3D at eps scale (coincident / collinear / coplanar) -- returns an empty
// vector (never throws). Impossible intermediate states (a corrupted surface:
// a doubly-claimed directed edge, a non-simple-cycle horizon, or a face count
// above the Euler bound 2n-4) abort with a diagnostic rather than looping.
// Impl in deltahedron.cc (the hull the deltahedron pipeline's
// project_onto_convex_hull builds, exported for warm-start convexification in
