...
{   
    ...
    explicit operator Graph() const {
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            for (size_t j = 0; j < degree; j++) {
                adj.push_back(i, A[i * (is_cubic ? 3 : 6) + j]);
            }
        }
        return Graph(adj);
    }
    ...
    explicit operator Polyhedron() const {
        ...
        auto A = d_.A_cubic_.template as_span<std::array<K, 3>>();
        auto X = d_.X_cubic_.template as_span<std::array<T, 3>>();
        for (size_t i = 0; i < N_; i++) {
            for (size_t j = 0; j < 3; j++) {
                neighbours.push_back(i, A[i][j]);
                points[i][j] = X[i][j];
            }
        }
        return Polyhedron(PlanarGraph(neighbours), points);
    }
