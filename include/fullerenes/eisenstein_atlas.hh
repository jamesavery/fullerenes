...
// build_atlas would use T) + transitions.  A.T and the per-cell claims stay
// empty -- only cells + trans are built, which is all the intrinsic dual
// reconstruction (no T, no 3D) consumes.  A folded cell on a noisy
