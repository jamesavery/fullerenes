...
string Polyhedron::to_xyz() const {
  ...
  s << N << endl;
  s << "# Created by libgraph from Fullerene (http://ctcp.massey.ac.nz/index.php?page=fullerenes)" << endl;
  for(int i=0; i<N; ++i){
...
string Polyhedron::to_mol2() const {
  ...
  s << "# Created by libgraph from Fullerene (http://ctcp.massey.ac.nz/index.php?page=fullerenes)\n"
    << "@<TRIPOS>MOLECULE\n"
    << "Fullerene\n"
    << "\t"<<N<<"\t"<<Nedges<<"\t0\t0\t0\n"
    << "SMALL\n"
