...
\title{PROGRAM FULLERENE (Version 4.2.2)\\ -- A Program for the Topological Analysis of Fullerenes --\\ USER'S MANUAL}
...
We also thank Prof. Ottorino Ori (Zagreb) for fruitful discussions.}
...
\item[{Iopt}]  Flag for force-field optimization (Default: 0)\\
  ...
  as soon as the gradient falls below a value of 10.
   ...
   \paramname{ihessian} = 1 then Hessian matrix is calculated and diagonalized. The first $3n_v-6$ eigenvalues should be greater than zero for a minimum.
   \paramname{iprinth}  = 1 print full Hessian matrix.
