...
or from a Goldberg-Coxeter 2-index transformation (triangulation algorithm, input files \filename{GCc20.inp}
to \filename{GCc500.inp}). The 12 pentagon indices can also be obtained from the \textit{fullerene database} if 
...
(input file \filename{DB1.inp}). Information of vertex connections (edges) can also be used as input (input files \filename{EDGEc20.inp} and \filename{EDGEc60.inp}).
...
see for example the input file \filename{DB1.inp}.
...
(subroutine \funname{BrinkmannFowler}) \cite{BrinkmannFowler03}, see for example input files \textit{CCc50EK.inp, CCc60SW.inp, CCc80YF.inp}.
