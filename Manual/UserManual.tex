...
%Figure 6
 \begin{figure}[htbp]
	\centering
     ...
     \caption{Force-field optimised structure of $I_h$-\C{2160} viewed with VMD \cite{vmd}.}
...
be addressed, and it was therefore decided to extend version 3 to a general purpose
program package and make it available to the scientific community.
...
ring spiral pentagon indices (RSPI) through either a 3D Tutte embedding (3D-TE) \cite{Tutte}
or the adjacency matrix eigenvector (AME) method \cite{Atlas}. One can also construct
the $n$-th leapfrog $GC_{1,1}^n[C_{n_v}]$ fullerene or a halma fullerene $GC_{k,0}[C_{n_v}]$
as special cases of the more general Goldberg-Coxeter transformation $GC_{k,l}$
with $k \geq l$, which is implemented only for \C{20}, i.e. $GC_{k,l}[C_{20}]$).
...
gradients for the harmonic oscillator force field\cite{Wu87} is implemented in the
...
we have not yet optimised the force constants to match experimental or calculated vibrational frequencies.
...
\item The Goldberg-Coxeter ($T^\mathrm{GC}_{k,l}$[\C{n}]) transformation for fullerenes
other than \C{20};
\item Symmetrization of optimised coordinates to point group symmetry (or a chosen
...
For this the atoms are rearranged (permuted) such that the distances are smallest. Note that this works well for optimised
coordinates, e.g. cartesian coordinates obtained from a force-field optimization.

In addition to the ring spiral algorithm by Fowler and Manolopoulos we
...
optimised global minimum for a fullerene, and the force-field optimization might
...
The fullerene structure can be optimised by force fields using a Fletcher-Reeves-Polak-Ribiere geometry optimization \cite{NumRec}
...
Figure \ref{pic:C380andC384}). Note that considerable improvements were made to optimise the ring spiral search algorithm, and if 
...
B3LYP aug-cc-pVDZ optimised up to \C{60}, cc-pVDZ up to \C{180}, and 6-31G for the rest. 
...
\item[{Iopt}]  Flag for force-field optimization (Default: 0)\\
  If \paramname{Iopt} = 1  then fullerene is optimised using the force field method of Wu et al. \cite{Wu87} within a 
  ...
  If \paramname{Iopt} = 2  Preoptimise with input force field, then optimise with standard Wu force field. This is especially useful for the \paramname{fcoulomb} input (see below).
...
are difficult to optimise if the start is bad, and one needs to play with the \paramname{fCoulomb} parameter (see for example {RSPIc380.inp}).
\end{description}
