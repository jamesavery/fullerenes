...
or from a Goldberg-Coxeter 2-index transformation of \C{20}, i.e. $GC_{k,l}(\C{20})$ with $k \geq l$ and $k > 0$
...
(subroutine \funname{BrinkmannFowler}) \cite{BrinkmannFowler03}, see for example input files \filename{CCc50EK.inp, GEOMc60SW.inp, CCc80YF.inp}.
...
\item[{IP}]  If set to 1, verbose output is be produced, i.e. the full distance matrix, all Hamiltonian cycles 
  ...
  is a dabgerous option and should be used only for small fullerenes.
...
to be set to zero to indicate the end of input. For an example see input file \filename{EDGEc20.inp}.  It is legal but not necessary to define same edges twice.\\
