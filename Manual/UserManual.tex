...
5) Symmetrization of optimized coordinates to point group symmetry; 
...
10) Perfect matchings, Kekul\'e structures and Clar sextets;
...
In the Makefile several compile flags are set.  Per default, the machine dependent compile flag \verb|-m64| is set (if you are
using a 32-bit environment you will need to change this to \verb|-m32|).  The optimization level is set to \verb|-O3|.  
If you require an atom number above 6800 (which can be set in \filename{source/config.f}) there will be arrays which are larger
...
(4.4.3, 4.4.4  and 4.6 on 64-bit Linux works, ) you can set the \verb|-mcmodel| and \verb|-mlarge-data-threshold| flags to adequate values
to enable large memory support.

The executable ``fullerene'' runs on a Linux/\acr{UNIX} as\\\\
...
There are several other topological indicators \cite{Cataldo,Caporossi} of which the Estrada index and mean topological distance, the Wiener index, the hyper-Wiener
...
\begin{multline}
  ...
  E_{\mathrm{extWu}} =
        ...
        + \frac{f_p}{2}     \sum_{j_p}^{\mathrm{60}} \left(\theta_{j_p} - \theta_p\right)^2 
        + \frac{f_h}{2}     \sum_{j_h}^{\mathrm{3n_v-60}} \left(\theta_{j_h} - \theta_h\right)^2 
...
%Figure 6
 \begin{figure}[htbp]
	\centering
  		 \includegraphics[width=0.9\textwidth]{C2160.png}
...
\bibitem{Caporossi} P. W. Fowler, G. Caporossi, and P. Hansen, \textit{J. Phys. Chem. A} \textbf{105}, 6232-6242 (2001).
