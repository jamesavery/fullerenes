...
and Analyzing Structures of Regular Fullerenes}, J. Comput. Chem., in press, 2013); Version 4.4.
...
\textit{Fullerene} is a general purpose program that creates the adjacency matrix for any fullerene isomer, 
...
The program is written in standard \acr{FORTRAN} and C++ and is easily implemented within a Linux/\acr{UNIX} environment.
...
minimization \cite{NumRec} with analytical gradients for the harmonic oscillator force field\cite{Wu87}
is implemented in the current version, providing 
...
From version 4.0 onwards (released July 2012) C++ routines embedded into the original \acr{FORTRAN} program incorporate
...
More information can be obtained in an up-coming review article \cite{PSJA}.
...
\begin{figure}[htbp]
    ...
    \caption{Simplfied diagram of the workflow.}
...
In addition to the ring spiral algorithm by Fowler and Manolopoulos we have implemented a general ring spiral algorithm for cubic graphs.
It differs from the classical ring spiral algorithm in that it can insert jumps between the addition of two vertices to the spiral in order
to prevent walking into a cul-de-sac at a later stage.  It is therefore capable of describing all fullerene graphs (and any other cubic graph).
The criterion for jumping (while deriving the spiral from a graph) is, that the
subgraph that hasn't been added to the spiral yet, must never be disconnected.  For deriving the graph from the spiral it is necessary
...
\item[{irext}] Flag for reading external input file which contains cartesian coordinates or connectivities (Default: 0). Note this makes the input of \paramname{ICart} obsolete if set non-zero.
  If \paramname{irext} = 1: Read \filename{.xyz} from file specified by \paramname{filename}. The final name is chosen as \filename{filename-3D.xyz} (Default: Fullerene-3D.xyz). 
  Another \filename{.xyz} file is also produced and named \filename{filename-3Dnew.xyz} if also \paramname{iwext} = 1 is specified. (Default: Fullerene-3D2.xyz)\\
  If \paramname{irext} = 2: Read \filename{.cc1} from file specified by \paramname{filename}. The final name is chosen as \filename{filename-3D.cc1} (Default: Fullerene-3D.cc1). 
  Another \filename{.cc1} file is also produced and named \filename{filename-3Dnew.cc1} if also \paramname{iwext} = 2 is specified. (Default: Fullerene-3D2.xyz).
  ...
  If \paramname{irext} = 3: Read \filename{.mol2} from file specified by \paramname{filename}. The final name is chosen as \filename{filename-3D.mol2} (Default: Fullerene-3D.mol2). 
  Another \filename{.mol2} file is also produced and named \filename{filename-3Dnew.mol2} if also \paramname{iwext} = 1 is specified. (Default: Fullerene-3D2.mol2).
...
\paramname{ICart, rspi, jumps, IV1, IV2, IV3, isonum, leap, IGCtrans, kGC, lGC, ISW, KE, IYF, ISW, mirror, IPRC, R5, R6, ScaleRad}
...
If \paramname{ICart} = 8 or 9 general spiral input required using  \paramname{rspi} and \paramname{jumps}.\\ 
...
\item[jumps] An array of up to 10 jump positions and step lengths.  \paramname{jumps} is required in case of \paramname=\{8,9\}
(unless you use the general spiral algorithm to create a graph that doesn't require jumps [which is possible of course]).
...
A database is provided for general isomers up to \C{110} and for IPR isomers up to
...
by the program. It is our intention to extend the isomer list beyond \C{140}/\C{184} (without Hamiltonian cycles) using the 
...
\bibitem{Tonner} R. Tonner, G. Frenking, M. Lein, and P. Schwerdtfeger, ``Packed to the Rafters - Filling up C60 with Rare Gas Atoms'', 
