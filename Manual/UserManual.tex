...
fullerene (see files \filename{CCc20.inp} to \filename{CCc540.inp}), or to construct them internally for the high
symmetry $I_h-$\C{20} or $I_h-$\C{60} isomers (Subroutine \funname{CoordC20C60}, see input files \filename{GEOMc60.inp},
\filename{GEOMc60exp.inp}, \filename{GEOMc60ideal.inp}), or to get the cartesian coordinates from input ring-spiral
pentagon-indices RSPI (Subroutine \funname{CoordBuild}) by using either the Fowler-Manolopoulus AME algorithm \cite{Atlas},
or the more reliable 3D-TE algorithm (used by files \filename{RSPIc20.inp} to for example \filename{RSPIc840.inp}),
or from a Goldberg-Coxeter 2-index transformation (triangulation algorithm, input files \filename{GEOMc20Halma.inp}
to \filename{GEOMc500Halma.inp}). The 12 pentagon indices can also be obtained from the \textit{fullerene database} if 
...
(e.g. input file \filename{DBc66.inp}). Information of vertex connections (edges) can also be used as input (input files \filename{EDGEc20.inp} and \filename{EDGEc60.inp}).
...
1) All files starting with \filename{CC} have cartesian coordinates for input, for example for
\C{20} (\filename{CCc20.inp}), \C{24} (\filename{CCc24.inp}), \C{60} (\filename{CCc60.inp}), or \C{540} (\filename{CCc540.inp}).
2) All files starting with \filename{GEOM} use basic geometry to construct \C{20} or \C{60}, e.g. \filename{GEOMc60.inp} or \filename{GEOMc60exp.inp}.\\
3) All files starting with \filename{EDGE} have vertex connectivities as input, e.g. \filename{EDGEc20.inp}.\\
4) All files starting with \filename{RSPI} have the ring-spiral pentagon indices as input, e.g. \filename{RSPIc20.inp} or \filename{RSPIc60NT.inp}
...
\begin{verbatim}
C80  Using FM algorithm to produce coordinates for C80 nanotube, input file: \filename{RSPIc80NT.inp}
&General NA=80 / 
...
to be set to zero. For an example see input file \filename{EDGEc20.inp}.  It is legal but not necassary to define edges twice.
...
\item[ftol] The convergence tolerance on the energy (Default: $5.0 \times 10^{-5}$ kJ/mol)
...
\bibitem{Tonner} R. Tonner, G. Frenking, M. Lein, P. Schwerdtfeger, ``Packed to the Rafters - Filling up C60 with Rare Gas Atoms'', 
...
\bibitem{kamada89} T. Kamada, S. Kawai, ``An Algorithm for Drawing General Undirected Graphs'', \textit{Inform. Process. Lett.} \textbf{31}, 7-15 (1989).
