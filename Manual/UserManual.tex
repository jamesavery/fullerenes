...
D_{\mathrm{MCS}} =\frac{100}{n_vr_{\min}} \sum_{i=1}^{n_v} \left(R_{\mathrm{MCS}} - \|\vec{p}_i - \vec{c}_{\mathrm{MCS}}\|\right)
\end{equation} 
...
\begin{equation} 
\mathop{\min }\limits_{c_{MDS} \in CH(S)} \frac{1}{n_v} \sum _{i}^{}\left|R_{MDS} -\left\| \vec{p}_{i}-\vec{c}_{MDS} \right\| \right|  
\end{equation}
...
symmetric. There are a number of different algorithms available to achieve this, and in subroutine {\bf Graph2D} $(X_i,Y_i) (i=1,\dots, n_v)$ 
coordinates for a 2D fullerene graph are created. Here we briefly describe the algorithms we recommend to use,
others are of perhaps of more historical interest. The subroutine produces latex file with the fullerene graph included.
...
\begin{equation} 
\label{eq:Eppg} 
E_{\mathrm{PPG}} =\sum _{i<j}^{ord(G)} A_{ij}  r_{ij}^{2} \exp \left(\alpha \frac{2d_{\mathrm{max}} -d_{iP} -d_{jP} }{d_{\mathrm{max}} } \right) 
\end{equation} 
...
{\it List of options:}\\ 
{\bf NA, ICart, IV1, IV2, IV3, ixyz, filename, isonum, ichk, leap, GCtrans, kGC, lGC, ISW, KE, IYF, ISW, ihueckel, loop, mirror, IP, IPRC, TolR, R5, R6}\\
