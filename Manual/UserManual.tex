...
\title{PROGRAM FULLERENE (Version 4.2)\\ -- A Program for the Topological Analysis of Fullerenes --\\ USER'S MANUAL}
...
A Fortran and C++ Program (Version 4.2)}, (Massey University Albany, 
...
15) F. Cataldo, A. Graovac, and O. Ori \textit{The Mathematics and Topology of Fullernes} (Springer, Berlin, 2011).\\
...
3) Geometry optimization using further improved force fields; 4) Frequency calculations from the force-field 
...
10) A restart option for subroutine Hamilton;  
11) Plotting program (other than latex) for producing Schlegel diagrams and corresponding duals
...
to be created and moved into the main directory where the \filename{source} and \filename{libgraph} directories are located. \filename{database} contains
...
and \filename{HOG} with (if downloaded from http://hog.grinvin.org/Fullerenes ) the House of Graph datafiles.
Important: Please make all filenames in the database read-only.
...
\begin{equation}
  \label{Balaban}
  B(G)=\frac{n_e}{n_e-n_v+1} \sum_{i<j \in E(G)} \left( W(i)W(j) \right)^{-1/2}
\end{equation}
...
\end{equation}
For $I_h$-\C{60} we have $B(G)$= 0.939893247. The Schultz and Zagreb indices are also listed, although they are related to
...
\begin{equation}
  \label{rho}
  \rho=\frac{W(G)}{n_v W_{\mathrm{min}}} \quad \text{ with } \quad W_{\mathrm{min}}= \min \{ W_i \}
\end{equation}
and
\begin{equation}
  \label{rhoE}
...
%Figure 2
 \begin{figure}[htbp]
   	\centering
  	\includegraphics[width=0.35\textwidth]{GoldbergCoxeter.png}
    \caption{Goldberg-Coxeter construction: $GC_{1,1}$ (green triangle), leapfrog transformation; $GC_{3,0}$ (blue triangle), halma transformation; $GC_{6,2}$ (red triangle), general.}
...
respectively). As the Wu force field was developed for $I_h$-\C{60} only adacent pentagons are treated in the same way as a pentagon adjacent to a hexagon.
...
The connections between the points are already written out earlier in the output such that the fullerene graph can be drawn. This is shown in Figure \ref{pic:Schlegel}.
...
\paragraph{List of options:}\  \paramname{NA, IP, ixyz, ichk, ihueckel, loop, ipsphere, nosort, TolR, R5, R6, filenamedb, filename}.
...
\item[IBF] 
If \paramname{IBF} = 1 the initial fullerene structure is subjected to a 9-vertex insertion (3 are deleted) (Default: 0).
...
need to be downloaded from the House of Graphs website) and read-in. 
...
\bibitem{Cataldo} F. Cataldo, A. Graovac, and O. Ori ``The Mathematics and Topology of Fullernes'', Springer, Berlin (2011).
