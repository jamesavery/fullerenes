...
2) The general Goldberg-Coxeter (GC$_{k,l}(n)$) construction for fullerenes other than C20;
...
the House of Graphs was made possible. Version 4.2 saw the introduction of dihedral angles into the force field,
and Version 4.3 was extended to calculate frequencies from the Hessian of the force field and introduced
the the Goldberg-Coxeter transformation $GC_{k,l}$ of \C{20} for all possible combinations of $k$ and $l$.
...
(triangulation algorithm described below, input files \filename{GEOMc20Halma.inp}
to \filename{GEOMc500Halma.inp}) and for example \filename{GGCK6L2.inp}. 
The 12 pentagon indices can also be obtained from the \textit{fullerene database} if 
...
to construct 3D fullerene graphs. For the Goldberg-Coxeter transformation of \C{20} we construct the RSPI from the scheme introduced by
Fowler and Rogers.\cite{Rogers}
For the AME algorithm it is critical to get the right 
...
\paragraph{List of options:}\  \paramname{NA, IP, ixyz, ichk, ihueckel, loop, ipsphere, nosort, nospiral, novolume, TolR, R5, R6, filenamedb, filename}.
...
%Figure 6
 \begin{figure}[htbp]
	\centering
  		 \includegraphics[width=0.5\textwidth]{C2160.png}
...
\bibitem{Rogers} P. Fowler and K. M. Rogers, ``Spiral Codes and Goldberg Representations of Icosahedral Fullerenes and Octahedral
