...
(input file {\it database.inp}). The program also print internal coordinates in form of a Z-Matrix (distances, angles and dihedrals).
...
The first line is a text line, the second \&General is a general command line containing many useful flags, 
the third \&Coord line in {\it namelist format} tells the program how the coordinates are created, the
...
line in the input is for the 12 ring-spiral pentagon indices (RSPI) of indices for vertex insertions.
...
In this case {\bf ICart} = 0  needs to be specified (see \&Coord input). This is useful for reading for example cc1 files produced by M. Yoshida \cite{Yoshida}.\\
...
If {\bf ipsphere} = 3 In addition an cc1 file is written out named filename-3DMCS.cc1 (Default: Fullerene-3DMCS.cc1).\\
...
7) {\bf \&Graph line}: Option for producing $(X_i, Y_i)$ coordinates for fullerene 2D graphs (Schlegel diagrams).\\
