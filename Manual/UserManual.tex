...
John Graham-Cumming, Nature {\bf 482}, p.485 (2012). {\it However, we kindly ask that if 
our program is used data are subsequently published, to cite the three references given below.
...
{\noindent\bf Please cite the following papers if you use this program for publishing data:}\\
...
{\noindent\bf Further suggested reading:}\\
...
The program produces an external file with default name {\bf Fullerene-3D.xyz} or {\bf Fullerene-3D.cc1} to be used for standard molecular plotting programs
...
pentagon-indices RSPI (Subroutine {\bf CoordBuild}) by using either the Fowler-Manolopoulus AME algorithm \cite{Atlas},
...
(input file {\it database.inp}). The program also prints internal coordinates in form of a Z-Matrix (distances, angles and dihedrals).
...
with analytical gradients (Subroutine {\bf OptFF} \cite{NumericalRecipes}). In the currently version only the Wu
...
insertions (subroutine {\bf YoshidaFowler6}) \cite{Yoshida97}, or a WS 6-vertex (6-55-55) insertion 
we developed, see input files {\it c50EK.inp, c60SW.inp, c80YF.inp}. 
Non-spiral fullerenes such as  \C{380} or  \C{384} can be constructed with such vertex insertion methods, see input files {\it c380.inp} 
...
The first line is a text line, the second \&General is a general command line in {\it namelist format} containing many useful flags, 
the third \&Coord line tells the program how the coordinates are created, the
next two lines \&FFChoice and \&FFParameters are specifications for the force field to be used (none in this case), line 5 (\&Hamilton) concerns the
Hamiltonian cycles, line 6 (\&Isomers) is an isomer list is created, and line 7 (\&Graph) for the 2D fullerene graph (Schlegel diagram). The last
line in the input is for the 12 ring-spiral pentagon indices (RSPI) (or for some other inputs the indices for vertex insertions).
...
2) {\bf \&General line}: General command line containing main flags for the program (e.g. \&General NA=60 /).\\
{\it List of options:}\\ 
{\bf NA, IP, ixyz, ichk, ihueckel, loop, ipsphere, nosort, TolR, R5, R6, filename}\\
...
Default name for the external file is {\it Fullerene-3D.xyz}. Otherwise {\bf filename}='filename' needs to be specified (max. 50 characters).\\
If {\bf ixyz} = 1: Produce .xyz file named {\bf filename-3D.xyz} for further input.\\
If {\bf ixyz} = 2: Read .xyz from file specified in {\bf filename}. The final name is chosen as {\it filename-3D.xyz} (Default: Fullerene-3D.xyz). 
{\bf ICart} = 0 needs to be specified for this. Another .xyz file is also produced and named   {\it filename-3Dnew.xyz}. (Default: Fullerene-3Dnew.xyz)\\
If {\bf ixyz} = 3: Read .xyz from file specified in {\bf filename}. The final name is chosen as {\it filename-3D.xyz} (Default: Fullerene-3D.xyz). 
{\bf ICart} = 0  needs to be specified.\\
If {\bf ixyz} = 4: Produce .cc1 file (Chem3D cartesian coordinate format 1 file) for further input. Note that in contrast to .xyz files,
.cc1 files contain information about vertex connections. Programs like PYMOL can read such files.\\
If {\bf ixyz} = 5: Read .cc1 from file specified in {\bf filename}. The final name is chosen as {\it filename-3D.cc1}. 
In this case ICart = 0  needs to be specified (see \&Coord input). This is useful for reading for example .cc1 files produced 
...
If {\bf ipsphere} = 2 In addition a .xyz file is written out named filename-3DMCS.xyz (Default: Fullerene-3DMCS.xyz).\\
If {\bf ipsphere} = 3 In addition a .cc1 file is written out named filename-3DMCS.cc1 (Default: Fullerene-3DMCS.cc1).\\
If {\bf nosort} = 1 permutation of vertices for better Z-Matrix is suppressed (Default: 0).\\
...
{\bf filename} = 'name'. Filename for all external files created or used by the program. Limited to 50 characters.\\\\

3) {\bf \&Coord line}: Input to create cartesian coordinates for the program (e.g. \&Coord ICart=20, R6=1.42 /).\\
{\it List of options:}\\ 
{\bf ICart, IV1, IV2, IV3, isonum, leap, IGCtrans, kGC, lGC, ISW, KE, IYF, ISW, mirror, IPRC, R5, R6}\\
...
definition is similar to the {\bf IPR} parameter below (Default: 2).\\There are two databases, one for the general isomers
...
If {\bf IGCtrans} = 1 the initial fullerene structure is subjected to a Goldberg-Coxeter transformation $GC_{k,l}$[C$_{N_A}$] (Default: 0).
...
4) {\bf \&FFParameters and \&FFParameters  lines}: Options for force-field optimization.\\
...
If {\bf Iopt} = 2  Preoptimize with input force field, then optimize with standard Wu force field. This is especially useful for the {\bf fcoulomb} 
input (see below).  \\
{\bf ftol}: The convergence tolerance on the energy (Default: $5.0 \times 10^{-8}$)
{\bf WuR5, WuR6, WuA5, WuA6, WufR, WufA} are force-field parameters for the Wu force field, i.e. for
...
a good guess of the initial structure. In such cases {\bf fCoulomb} = 100.0 is perhaps a good choice. Note that some fullerene structures
are difficult to optimize if the start is bad, and one needs to play with the {\bf fCoulomb} parameter (see for example {c380.inp}).\\\\

5) {\bf \&Hamilton line}: Option for calculating Hamiltonian cycles and IUPAC numbers.\\
...
6) {\bf \&Isomers line}: Option for producing list of isomers and properties.\\
...
7) {\bf \&Graph line}: Option for producing $(X_i, Y_i)$ coordinates for fullerene 2D graphs (called Schlegel diagrams by some authors).\\
...
If {\bf ISchlegel} = 6: Starting from the Tutte graph perform spring + repulsive Coulomb with prefactor of 0.3 for the
...
This is much faster than their simulated annealing algorithm.\\
...
folder {\it database/Yoshida} \cite{Yoshida}. The input file {\it read1.inp} shows an example for reading from
