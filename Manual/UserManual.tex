...
their Fortran subroutines which were modified and implemented within this program.
...
minimization with analytical gradients for the Wu (and extended Wu) force-field \cite{Wu87} is implemented in the current version, providing 
...
\begin{equation}
  \label{Balaban}
  B(G)=\frac{n_e}{n_e-n_v+1} \sum_{i<j \in E(G)} \left( d(i)d(j) \right)^{-1/2}
\end{equation}
$n_e$ are the number of edges and $d(i)$ is the sum of topological distances between vertex $i$ and all other vertices in the graph.
...
\begin{equation}
  \label{rho}
  \rho=\frac{W(G)}{n_v W_{\mathrm{min}}} \quad \text{ with } \quad W_{\mathrm{min}}= \min \{ W_i \}  \quad \text{ and } \quad W_i=\sum_{j \in V(G)} d(i,j)
\end{equation}
and
\begin{equation}
  \label{rhoE}
...
If \paramname{ICart} = 2 or 3 and \paramname{isonum} $\neq 0$, than pentagon indices are taken from the isomer list contained in a database (see below).
\item[{IPRC}]
