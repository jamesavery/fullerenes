...
their Fortran subroutines which are implemented in our program.\\\\
...
{\it Program Fullerene} is a general purpose program that creates a 3D structure (cartesian coordinates) for any fullerene isomer 
through various different methods and algorithms, and subsequently performs a topological/graph 
...
like {\it Fullgen} \cite{Brinkman}, {\it Buckygen} \cite{Goedgebeur} or VEGA \cite{Pisanski},
...
fulfilling Euler's theorem, $n_v -n_e +n_f =2$, where $n_v$ are the number of vertices (atoms), $n_e$ the number of edges (bonds), and 
...
from the canonical ring spiral pentagon indices (RSPI) through either a Tutte embedding (TE) \cite{Tutte} or the adjacency matrix
eigenvector (AME) method \cite{Atlas}. One can also construct the n-th leapfrog $GC_{1,1}^n$[C$_{n_v}$] fullerene or the halma fullerenes 
$GC_{k,0}$[C$_{n_v}$] as a special case of a Goldberg-Coxeter transformation $GC_{k,l}$. Starting from a given fullerene structure one can 
perform vertex insertions (such as Endo-Kroto \cite{Endo92}) or Stone-Wales transformations \cite{Stone86}. 
Vertex insertions allow for the construction of non-spiral fullerenes. 
...
a good initial guess for 3D cartesian coordinates. Schlegel diagrams (2D graphs)
...
total resonance energy of a general fullerene \cite{Babic1995,Babic1997}.\\
...
The program produces an external file with default name {\bf cylview.xyz} to be used for standard molecular plotting programs
like CYLview \cite{CYLview}, Avogadro \cite{Avogadro}, Jmol \cite{JMol}, or Pymol \cite{Pymol}. Note for using these programs 
it is important that the fullerene has a reasonable structure, obtained for example by a force-field 
optimization, otherwise bonds cannot be correctly identified. It may not be advisable to use structures that come out directly from
AME or 3D-TE algorithm. We recommend CYLview, it is more robust and works for the large fullerenes up to about 1000 atoms. 
For larger fullerenes Pymol is recommended. The program produces fullerene 2D graphs in latex format, but you can
be used any other program which is capable of producing 2D graphs, for example QMGA \cite{Gabriel2008}. 
For this an external file is written out called {\bf qmga.dat}.\\
...
and geometry optimization. This program is under permanent construction and has been tested for bugs.
Nevertheless, if you have any problem or find any bug please report it to one of us.\\

There are many things on our to-do list. We try hard to release more functionality to the program soon.
Not implemented yet is: 1) The minimum covering ellipsoid useful for  rugby-ball shaped fullerenes (yes this is New Zealand
and crazy about rugby), and close packing of ellipsoids; 2) The general Goldberg-Coxeter construction for fullerenes;
3) Geometry optimization using an extended Wu-Fowler force field including dihedrals; 4) Frequency calculations from the force-field 
...
H\"uckel orbital energies; 7) Extend to non-regular fullerenes of genus 0 (heptagons and squares); 8) Extend to non-regular fullerenes 
of genus 1; 9) Symmetrize coordinates to point group symmetry; 10) Restart option for subroutine Hamilton; 11) Use of ``House of Graphs" 
fullerene database; 12) Plotting program (other than latex) for producing Schlegel diagrams and corresponding duals. We are also
...
All fortran files are situated in the directory "source" and all C-files are in "libgraph".
The program is LINUX/UNIX based and works fine with {\bf gfortran} and {\bf gc} compilers. You need to use the {\bf Makefile} included in 
the {\bf fullerene.zip} file provided and type\\\\ 
{\bf make}\\\\
...
{\bf ./fullerene $<$filename.inp $>$filename.out}\\\\
...
{\bf make clean}\\\\
all the object files are deleted. A bit more is deleted if you type\\\\
{\bf make distclean}\\\\
...
The most time consuming steps in the program are the diagonalization of the adjacency matrix needed for the AME algorithm and the H\"uckel analysis, 
the force-field optimization for very large fullerenes ($>$ 2000 atoms), the determination of the number of Hamiltonian cycles ($>$ 80 atoms), 
the creation of a list of ring-spiral pentagon indices for all possible isomers with a given vertex count $n_v$ ($>$ 80 atoms), 
and the Tutte embedding algorithm ($>$ 2000 atoms).

\subsection{Create a 3D structure for a specific fullerene}
The easiest way to create a 3D structure is of course to read-in cartesian coordinates for a specific fullerene 
(see files {\it c20.inp} to {\it c540.inp}), or to construct them internally for the high symmetry 
$I_h-$\C{20} or $I_h-$\C{60} isomers (Subroutine {\bf CoordC20C60}, see input files {\it ico.inp}, {\it icoExp.inp}, {\it icoideal.inp}), or to 
get the cartesian coordinates from input ring-spiral pentagon-indices PSPI (Subroutine {\bf CoordBuild}) by using either the Fowler-Manopoulus 
AME algorithm (the adjacency-matrix eigenvector algorithm) \cite{Atlas}, or the more reliable 3D Tutte embedding algorithm (3D-TE) 
(used by files {\it pentagon1.inp} to {\it pentagon28.inp}), or from the Goldberg-Coxeter 2-index triangulation algorithm (input files {\it coxeter1.inp}
to {\it coxeter5.inp}). The pentagon indices can also be read-in from the {\it fullerene
database} if the canonical fullerene number is used as defined in The book ``Atlas of Fullerenes" Fowler and Manopoulus \cite{Atlas,cvetkovic2002} 
(input file database.inp).
We recommend reading this book for the use of RSPIs and H\"uckel (adjacency matrix) $P$-type eigenvectors 
to construct 3D fullerene graphs. Note for the AME algorithm it is critical to get the right 
...
It is however important that the end-product is viewed by a molecular visualization program. 
We recommend CYLview by Claude Legault \cite{CYLview}, Avogadro \cite{Avogadro}, JMol \cite{JMol} or Pymol \cite{Pymol}. 
These codes are all freely downloadable. For this purpose a file is written out to cylview.xyz to be used as an input file 
for these programs. The program sets the barycenter of the fullerene 
...
moment of inertia, which already give a measure for distortion from spherical symmetry (Subroutine {\bf Diameter}). 
...
Once the cartesian coordinates $(X_i, Y_i, Z_i)$ for each vertex $i=1, ..., n_v$ is known, all edges are determined by 
...
required. Note that the number of isomers scale polynomially as $n_v^9$, thus it becomes soon computationally
too demanding (which happens beyond \C{100}). Moreover, the database provided can be used to print all isomers up to 
\C{100} and up to \C{120} for all IPR isomers. Fowler and Manolopoulus use a numbering scheme ($N_{isomers}= 1, ..., n_{max}$) 
obtained from the natural sequence of canonical RSPIs to distinguish between the different isomers \cite{Atlas}. From the book the
...
The numbering scheme for IPR isomers only ($N_{isomers}^{IPR}= 1, ..., n_{max}$) can also be used up to \C{120}, see input file {\it database.inp}.

\subsection{H\"uckel analysis}
From the adjacency matrix $A_{ij}$ a H\"uckel analysis is performed (Subroutine {\bf Hueckel}). This gives you a good hint if the 
fullerene is open or closed shell \cite{Atlas}. Note that for fullerenes the H\"uckel analysis may not be very reliable 
...
on the HOMO-LUMO gap and calculated the topological resonance energy from Babi\'c's matching polynomial calculations \cite{Babic1995,Babic1997}.
...
The general Goldberg-Coxeter construction \textit{GC${}_{k,l}$}[\textit{G}${}_{0}$] goes as follows: ($i$) consider a 3-valent plane graph \textit{G}${}_{0}$ and take its dual (faces become vertices and vertices become faces), \textit{D${}_{t}$}(\textit{G}${}_{0}$). This transforms the graph \textit{G}${}_{0}$ with $n_v$ vertices into a triangulation, i.e. a geodesic sphere with $n_v$ triangles. (\textit{ii}) each triangle \textit{T${}_{k}$} (\textit{k}=1,\dots ,\textit{ n${}_{v}$}) of the geodesic sphere is subdivided into another set of faces \textit{t${}_{i}$} using the Coxeter indices (\textit{k},\textit{l}). The trick with this construction is that if the obtained new smaller faces \textit{t${}_{i}$} are not triangles themselves within one \textit{T${}_{k}$}, when neighboring \textit{T${}_{k}$}`s are glued together with other non-triangle faces new triangles are formed and we end up with a new triangulation, i.e. a larger geodesic sphere \textit{GC${}_{k,}$}${}_{l}$\textit{D${}_{t}$}(\textit{G}${}_{0}$). (\textit{iii}) Finally we take the dual end up with a new fullerene,
\begin{equation}
\label{eq:GC}
  G_1 = GC_{k,l}[G_0] = D_t GC_{k,l} D_t(G_0)
\end{equation}
If the initial graph $G_0$ has $n_v$ vertices, the number of vertices of $GC_{k,l}[G0]$ is
\begin{equation}\label{eq:GCvertexcount} 
  n_v^{GC} = t(k,l)n_v = (k^2 + kl + l^2)n_v
\end{equation}
$t(k,l)$ is called the {\em triangulation number}. We work with the dual structure as it is easier.
...
to the starting point. The number of Hamiltonian cycles given has been checked 
...
for the larger fullerenes. The existence of Hamiltonian cycles for fullerenes is conjectured, but only for layered fullerenes 
(e.g. fullerene with onion-shaped 2D graphs such as nanotubes) it has been proven to exist. Our calculations proved that
they exist for all fullerene isomers up to \C{100} and all IPR isomers up to \C{120} \cite{PSAJDB}.
...
gradients (Subroutine {\bf OptFF} \cite{NumericalRecipes}). At the moment only the Wu force field is implemented which considers bond lengths and
angles in an harmonic oscillator approximation \cite{Wu87}: 
\begin{equation}
  \label{eq:Ewu}
  E_{Wu} = \frac{k_p}{2} \sum_{i_p}^{edges} \left(R_{i_p} - R_p\right)^2 
        + \frac{k_h}{2} \sum_{i_h}^{edges} \left(R_{i_h} - R_h\right)^2 
        + \frac{f_p}{2} \sum_{j_p}^{ring} \left(\theta_{j_p} - \theta_p\right)^2 
        + \frac{f_h}{2} \sum_{j_h}^{ring} \left(\theta_{j_h} - \theta_h\right)^2 
\end{equation}
$i_p$ and $i_h$ are restricted to the case where the adjacency matrix $A_{ij}$ for the atoms has nonzero values. $k_{p}$ and $k_{h}$ are
the force constant for the two different C---C bonds (set to $\approx 300 ~$kcal \AA$^{-2}$), $R_p$ and $R_h$ the corresponding pentagon and
...
lead to a more spherical arrangement, e.g. barrels instead of nanotubes which come out of the force-field optimization. 
A more sophisticate force field using dihedral angles is currently being developed.\\
...
Information from the subroutine {\bf RingC} can be used to perform Stone-Wales transformations (subroutine {\bf StoneWalesTrans}) \cite{Stone86}, 
Endo-Kroto 2-vertex insertions (subroutine {\bf EndoKrotoTrans} \cite{Endo92}, Yoshida-Fowler 4-vertex ($D_{3h}$ 6555) (subroutine {\bf YoshidaFowler4}) 
and 6-vertex ($D_{3h}$ 666555) insertions (subroutine {\bf YoshidaFowler6}) \cite{Yoshida97}, or the a WS 6-vertex (6-55-55) insertion 
we developed, see input files {\it c50EK.inp, c60SW.inp, c80YF.inp}. 
The non-spiral fullerenes such as  \C{380} or  \C{384} can be constructed with these vertex insertion methods, see input files {\it c380.inp} 
and {\it c384.inp}.
...
A general ring-spiral algorithm is in preparation.
...
polyhedron is convex). Here one sums over all tetrahedrons spanned by the three vectors CM-CR (barycenter of fullerene to the ring center),
...
known analytically.\\
The {\it isoperimetric quotient} (IPQ) defined as
 \begin{equation}
 \label{IPQ}
   q_{IPQ} =36\pi \frac{V^{2} }{S^{3} } \ \text{  with }\   q_{IPQ} \in [0,1]
 \end{equation}
is obtained, which for an ideal sphere is trivially \textit{q${}_{IPQ}$}${}_{ }$= 1, and for zero volume \textit{q${}_{IPQ}$}${}_{ }$= 0. For \C{20} 
...
algorithm of Yildirim \cite{Yildirim08}, minimum distance sphere (MDS) (Subroutine {\bf MinDistSphere}), and the maximum inner sphere (MIS)
(Subroutine {\bf MaxInSphere}). Let $S = \vec{p}_i$ be the set of $n$ points ($i=1,\ldots ,n$) in $m$-dimensional Euclidean space, 
...
\begin{equation}
\label{eq:MCS}
\mathop{\min }\limits_{\vec{c}_{MCS} } \mathop{\max }\limits_i \left\| \vec{p}_i -\vec{c}_{MCS} \right\|  
\end{equation} 
...
We can now introduce the definition for the MCS distortion parameter $D_{MCS}$ (in \% of $r_{\min}$),
\begin{equation} \label{eq:DMCS}
D_{MCS} =\frac{100}{n_vr_{\min}} \sum_{i=1}^{n_v} \left(R_{MCS} - \|\vec{p}_i - \vec{c}_{MCS}\|\right)
\end{equation} 
At the end the Van der Waals radius of carbon (1.415 \AA) is added to the radius of the mMCS 
...
are reproduced.
Note that in the case of nonplanar 5- or 6-rings there is no unique definition for the 
volume of a fullerene except for the convex hull. There is no reason however why any other definition than the 
fast tesselation algorithm used here should be preferred. \\

The minimum distance sphere (MDS) of a fullerene is biased for the case that few atoms stick 
out on a sphere and the MDS may be more appropriate for a measure from spherical distortion. The MDS radius is defined as
\begin{equation} 
\label{eq:RMDS}
R_{MDS} =\frac{1}{N} \sum _{i}^{}\left\| \vec{p}_{i} -\vec{c}_{MDS} \right\|   
\end{equation} 
The restriction of $\vec{c}_{MDS}$ to be inside the convex hull is necessary as the center may move out of the fullerene structure in the optimization. One can now easily verify this definition is different from the MCS and generally $\vec{c}_{MDS}\ne\vec{c}_{MCP}$. This is a much better distortion parameter compared to $D_{MCS}$, as it is not biased to a few points lying outside or inside the sphere.
The MDS may not be uniquely defined, as there can be many (even degenerate) local minima, but for most spherical fullerenes it should 
just be fine. Analogous to the MCS there will be a measure for distortion from spherical symmetry,
For the distortion parameter we get
\begin{equation}
  \label{eq:DMDS}
  D_{MDS} = \frac{100}{N r_{\min}} \sum_{i=1}^N \left|R_{MDS} - \|\vec{p}_i - \vec{c}_{MDS}\| \right|
\end{equation}
\\

The maximum inner sphere (MCS) of a fullerene is important to estimate if there is space inside the cage for endohedral atoms or molecules. It is
defined as
\begin{equation} \label{MES} 
\mathop{\max }\limits_{c_{\mathrm{MES}} \in \mathrm{CH}(S)} \mathop{\min }\limits_{i} \left\| \vec{p}_{i} -\vec{c}_{\mathrm{MES}} \right\|  
\end{equation}
 For ideal \C{60} ($I_h$ symmetry) the MCS and MES are identical. 
For this the radius and volume is printed out with the Van der Waals
radius of carbon taken off $R_{MDS}$. 
...
symmetric. In Subroutine {\bf Graph2D} $(X_i,Y_i) (i=1, ..., n_v)$ coordinates 
of a fullerene graph is created. There are a number of different algorithms available to achieve this. We list here the ones we recommend to use,
others are of more historical interest.

1) The perspective Schlegel projection (SP): Here the points are rotated so to put a selected vertex, edge or ring center 
on top of the $z$-axis just below the projection point (otherwise the point $(0,0,z_{max})$ is chosen with $z_{max}$
...
3) Tutte 2D embedding with linear scaling (2D-TE-LS). Here the Tutte algorithms is used \cite{Tutte} but the resulting overcrowding overcrowding of small polygons near the center of the graph is remedied by a linear scaling procedure
 \begin{equation}\label{GrindEQ__16_} 
  \lambda_i = 1+\frac{f(r_{min} - r_i)}{r_{min}}
\end{equation}
applying a linear scaling factor $\lambda_i$ to all vertices $v_i$ ($i=5(6),\dots , n_v$), where $f$ is a parameter to be determined, $r_{\min}$ is the smallest distance to the vertices of the peripheral taken from the barycenter $P$ of the innermost ring (or alternatively the barycenter of the convex hull), and $r_i$ is the distance from that point $P$ to the vertex $v_i$.
...
\begin{equation} 
\label{eq:Eppg} 
E_{PPG} =\sum _{i<j}^{ord(G)} A_{ij}  r_{ij}^{2} \exp \left(\alpha \frac{2d_{\max } -d_{iP} -d_{jP} }{d_{\max } } \right) 
\end{equation} 
where $A_{ij}$ is the adjacency matrix, $r_{ij}$ the distance between vertex $i$ and $j$, and $a$ a parameter to be determined. The distances in eq.\eqref{eq:Eppg} are defined through the topological distance matrix $D$,
\begin{equation}\label{GrindEQ__20_}
  d_{\max} = \max_{j,j_P}\left\{ D_{ij_P} \right\}
  \ \ \text{ and }\ \ 
  d_{ip} = \min_{j_P}\left\{ D_{ij_P} \right\}
\end{equation}
where  $j_P$ is the vertex number belonging to the (5 or 6) peripheral vertices. Note that instead of a simulated annealing algorith we start with a Tutte 2D graph and perform a minimization procedure for $E_{PPG}$.
...
Note that for some of the fullerenes listed the singlet state chosen may not be the electronic ground state.
...
{\it List of options:}\\ 
{\bf NA, IC, IV1, IV2, IV3, ixyz, xyzname, isonum, ichk, leap, GCtrans, kGC, lGC, ISW, KE, IYF, ISW, ihueckel, loop, mirror, IP, IPRC, TolR, R5, R6}\\
...
Default name for the external file is {\it cylview.xyz}. Otherwise {\bf xyzname}=filename needs to be specified (max 20 characters).\\
...
{\bf ichk} = Flag for producing or reading a .xyz file for CYLview, Pymol, Avogadro or other plotting ptograms (Default: 0).\\
If {\bf ixyz} = 1:  Produce xyz file for further input.\\
If {\bf ixyz} = 2:  Read xyz from file specified in xyzname. {\bf IC} = 0 needs to be specified for this. Another xyz file is also produces 
and named   {\it cylviewnew.xyz}.\\
If {\bf ixyz} = 3  Read xyz from file specified in xyzname. {\bf IC} = 0  needs to be specified.\\
{\bf xyzname}: A name of up to 16 characters must be chosen, the file is then named name.xyz (Default: cylview.dat).
...
Note that at the moment only $l=0$ is implemented, also the leapfrog transformation $k=1$ and $l=1$ is available in this input.
In this case {\bf kGC} in the namelist input has to be specified. \\
If {\bf ISW} = 1 the initial fullerene structure is subjected to a Stone-Wales transformation (Default: 0). In this case an
...
If {\bf IC} = 4 or 5 adjacency matrix is created from a Goldberg-Coxeter transform of \C{20}, i.e. $GC_{k,l}$[\C{20}]. Note at the moment
...
Fletcher-Reeves-Polak-Ribiere algorithm. There are more sophisticated force fields available \cite{Ceulemans},
...
Note that a converged energy much greater than zero implies that the set distances and angles in the Wu force field cannot be
...
If {\bf fCoulomb} $>$ 0. then add an additional repulsive Coulomb force from the barycenter to all atoms (Default:  {\bf fCoulomb} = 0.d0)
...
Note only with this option together with {\bf IP} = 1 in the \&Coord input all Hamiltonian cycles are printed out. 
...
\&Isomers options: {\bf IPR}, {\bf IPH}, {\bf IStop}, {\bf IChk}, {\bf chkname} (Default 0 for all options and 'checkpoint' for chkname).\\
...
If {\bf IPR} = 2 for general isomers (note that this generates a large output and takes some computer time for fullerenes larger than \C{80}).
Also the database has all this information (see below) up to \C{100}.\\
If {\bf IPH} = 1 then number of distinct Hamiltonian cycles is calculated for every isomer (note this is computer time extensive).\\
If {\bf istop} = 1 program stops after calling this routine.\\
If {\bf IChk} = 1  Restart: Isomer list is continued from previous output file called 'checkpoint' as default if not otherwise give in {\bf chkname}.
This is a restart option from a previous run which terminated. Note that the new output file does not contain the previous one.
...
\&Graph options: {\bf ISchlegel, ISO1, ISO2, ISO3, PS, Scale, ScalePPG, graphname} (Default: 0 for all values except for {\bf graphname}). We recommend {\bf IG} = 1, 2, 4 or 7.\\
If {\bf ISchlegel} $>$ 0 Use Program Schlegel for generating fullerene graphs
If {\bf ISchlegel} = 1: Use the perspective projection method (PSP).\\
...
If {\bf ISchlegel} = 6: Starting from the Tutte graph perform spring + repulsive Coulomb.
...
(called Schlegel by the authors). Note: This is much faster than their simulated annealing algorithm.\\
If {\bf ISchlegel} = 8: Starting from the Tutte graph perform a Kamada-Kawai embedding 
optimization using the distance matrix (2D-KKE). Note: This gives a 2D picture of a 3D structure, thus
...
to the projection plane, which touches the point with the smallest $z$-value (opposite the projection point). Note that angle is reset 
to 45 degrees if chosen larger than 89 degrees. In the case of the perspective projection PS is the distance
between the focal point and the ring center underneath, Note this is chosen by the program, but if nonzero this parameter has to be
carefully chosen. The picture produced gives a good idea if the {\bf PS} is correctly chosen or not. Note that the ring centers get
...
{\bf graphname} = name. A name of up to 16 characters including at the end must be chosen, the file is then named name.dat (Default: graph2D.dat).
...
\bibitem{Tutte} W. T. Tutte, {\it Proc. London Math. Soc.} 13, 743Ð767 (1963).

\bibitem{Atlas} P. W. Fowler and D. E. Manolopoulus, ``An Atlas of Fullerenes" (Dover Publ., New York, 2006).
 
\bibitem{Endo92} M. Endo and H.W. Kroto, {\it J. Phys. Chem.} {\bf 96}, 6941-6944 (1992).
...
\bibitem{HouseofGraphs} The House of Graphs website at https://hog.grinvin.org/Fullerenes
...
\bibitem{cvetkovic2002} D. Cvetkovi\'c, P. Fowler, P. Rowlinson, and D. Stevanovi«c, {\it Linear Algebra Appl.} {\bf 356}, 37Ð56 (2002).

\bibitem{Babic1995a} D. Babi\'c, ``Nomenclature and Coding of Fullerenes", {\it J. Chem. Inf. Comput. Sci.} {\bf 35}, 515-526 (1995).

\bibitem{PSAJDB} P. Schwerdtfeger, D. Babi\'c, and J. Avery, in preparation.
...
\bibitem{Yoshida97} M. Yoshida, P. Fowler, {\it Chem. Phys. Lett.} {\bf 278}, 256Ð261(1997).
...
\bibitem{Heiney91} P.A.Heiney et al., {\it Phys. Rev. Lett.} {\bf 66}, 2911 (1991).

\bibitem{pisanski95} T. Pisanski, B. Plestenjak, and A. Graovac, {\it Croat. Chem. Acta} {\bf 68}, 283Ð292 (1995).
...
\bibitem{ands} I. D. Hands, J. L. Dunn, and C. A. Bates, {\it J. Chem. Phys.} {\bf 120}, 6912-xxx (2004).
