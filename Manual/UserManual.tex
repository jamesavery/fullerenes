...
1) P. Schwerdtfeger, L. Wirz, and J. Avery, \textit{Fullerene -- A Software Package for Constructing and Analyzing 
Structures of Regular Fullerenes}, submitted to J. Comput. Chem., 2013); Version 4.4.\\
...
coordinates) through various different methods and algorithms. The results can be used for plotting 2D fullerene graphs (Schlegel diagrams) 
...
like \program{CaGe} \cite{Brinkmanx}, \program{FULLGEN} \cite{Brinkmann}, \program{BUCKYGEN} \cite{Goedgebeur,Goedgebeur1}, FuiGui \cite{FuiGui} 
or VEGA \cite{Pisanski}, we felt that there is a need to write a general purpose program which does what other programs do and (of course) a lot more.
...
\begin{itemize}
\item The minimum covering ellipsoid useful for  rugby-ball shaped fullerenes (yes this is New Zealand and crazy about rugby), and close packing of ellipsoids; 
\item The general Goldberg-Coxeter (GC$_{k,l}(n)$) construction for fullerenes other than \C{20};
\item Construction of non-ring-spiral isomers using a generalized ring spiral algorithm; 
\item Symmetrization of optimized coordinates to point group symmetry; 
\item Symmetry labels for H\"uckel orbital energies and vibrational frequencies; 
\item Extension to non-classical fullerenes of genus 0 (heptagons and squares); 
\item Extension to non-regular fullerenes of genus 1 (hmmm, donuts!); 
\item A restart option for subroutine Hamilton;
\item Perfect matchings, Kekul\'e structures and Clar sextets;
\item Plotting program (other than latex) for producing Schlegel diagrams and corresponding duals with a good graphical interface similar to the \program{CaGe} program \cite{Brinkmanx}.
\end{itemize}
...
The number of spirals in the output agree with the examples given by Yoshida and Fowler.\cite{Yoshida97} 
...
up to \C{140} for general and up to \C{180} for IPR isomers. The database should be copied into
...
constructed easily from our database. Example is given in input files  \filename{DBc40.inp},
...
intention to extend the isomer list beyond \C{140}/\C{180} (without Hamiltonian cycles) using the ``House of Graphs'' database. 
...
\bibitem{Yoshida97} M. Yoshida and P. Fowler, ``Systematic relationships between fullerenes without spirals'', \textit{Chem. Phys. Lett.} \textbf{278}, 256-261(1997).
