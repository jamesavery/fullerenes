...
(used by files {\it pentagon1.inp} to {\it pentagon28.inp}). The pentagon indices can also be read-in from the {\it fullerene
...
known analytically. The routine also calculates the exact volume through a different tesselation method .... Also an algorithm
has been implemented to get the convex hull (CH). The {\it isoperimetric quotient} defined as
 \begin{equation}
 \label{IPQ}
...
Input and output files are in the directories {\it input}  and   {\it output}  respectively. {\it Program Fullerene} has been tested for the ideal capped 
for many fullerenes which found in the following input files:
...
\C{180} (c180.inp), \C{320} (c320.inp), and \C{540} (c540.inp), pentagon1.inp, ..., pentagon25.inp for
...
1) Text line of max. 132 characters (in 132A1 format) (you can put as many text cards into the input as you want)\\\\

2) {\bf \&Coord line}: Input to create cartesian coordinates and main flags for the program (e.g. \&Coord IC=20, IOPT=1, R6=1.42 /).\\
List of options:\\ 
{\bf NA, IC, IV1, IV2, IV3, ixyz, xyzname, isonum, ichk, leap, GCtrans, kGC, lGC, ISW, KE, IYF, IWS, ihueckel, loop, mirror, IP, IPRC, TolR, R5, R6, }\\
NA = Number of atoms (vertices) $N_A$ (Default: 60)\\
IC = Flag for the construction of the 3D structure (cartesian coordinates) (Default: 0). For more details see below.\\
...
ixyz = Flag for producing xyz input file for CYLview, Avogadro or others in standard xyz format (Default: 0). 
Default name for the external file is cylview.xyz . Otherwise xyzname=filename needs to be specified (max 20 characters).\\
isonum= Isomer number according to the scheme of Fowler and Manopoulus (Default: 0).\\
ichk = 1 Restart isomer list.\\
...
If leap = $n_{leap}$ the initial fullerene C$_{N_A}$ is subjected to an $n-th$ leapfrog transformation. 
This converts the number of atoms $N_A$ to $N^{'}_A = 3^{n_{leap}}N_A$. (Default: 0)\\
If GCtrans = 1 the initial fullerene structure is subjected to a Goldberg-Coxeter transformation $GC_{k,l}$[C$_{N_A}$] (Default: 0).
Note that at the moment only $l=0$ is implemented, also the leapfrog transformation $k=1$ and $l=1$ is available in this input.
In this case kGC in the namelist input has to be specified. \\
If ISW = 1 the initial fullerene structure is subjected to a Stone-Wales transformation (Default: 0).\\
If KE = 1 the initial fullerene structure is subjected to a Stone-Wales transformation (Default: 0).\\
If IYF = 1 the initial fullerene structure is subjected to a Yoshida-Fowler 4- or 6-vertex insertion (Default: 0).\\
If IWS = 1 the initial fullerene structure is subjected to a 9-vertex insertion (Default: 0).\\
If ihueckel = 1 diagonalization of H\"uckel matrix is avoided which is of order $O(n^3)$ (efault: 0)\\
If loop = 1 the program does not stop at the end but loops back to the beginning (Default: 0). This is important if one takes
an initial structure and subjects it to further transformations, for example in subsequent Stone-Wales or
Goldberg-Coxeter transformations.\\
If mirror = 1 invert all coordinates (take the mirror image). This may be important for chiral fullerenes (Default: 0).\\
If IP = 1 a larger output produced, i.e. the full distance matrix, all Hamiltonian cycles and all 3-ring connections (Default: 0).\\
TolR = Tolerance in \% (Default: 33). Only change this parameter if the program cannot produce correctly the adjacency matrix. 
...
\bibitem{JMol} JyMOL, ``a Java-based development component", see http://jmol.sourceforge.net/.

\bibitem{Pymol}. Program Pymol, ``A user-sponsered molecular visualization system on an open-source foundation", see http://www.pymol.org/.
...
\bibitem{cvetkovic2002} D. Cvetkovi\'c, P. Fowler, P. Rowlinson, and D. Stevanovi«c, {\it Linear Algebra Appl.} 356, 37Ð56 (2002).
...
\bibitem{pisanski95} Pisanski, T.; Plestenjak, B.; Graovac, A. Croat. Chem. Acta 1995, 68, 283Ð292.
