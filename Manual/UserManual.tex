...
\title{\program{Fullerene} (Version 4.6) \\A Program for the Topological Analysis of Fullerenes \\ User's Manual}
...
Version 4.6 contains bug fixes and also allows for subsequent GC transformations.
...
\textit{leapfrog transformations} ($k=l=1$) and \textit{halma transformations} ($k \neq 0$ and $l=0$ or $k=0$ and $l \neq 0$) for any fullerenes as implemented
in version 4.4, but also now for any pair indices and fullerenes, i.e.  $GC_{k,l}(\C{n})$) except for $k=l=0$. 
...
Note that negative values of $k$ or $l$ are allowed as this is a simple rotation in the Eisenstein plane. If the fullerene graph
belongs to one of the chiral point groups $G_C\subset G_F$ out of the possible set of
groups $G_F$ for fullerenes, then the two $GC_{k,l}[G_C-C_n]$ and $GC_{l,k}[G_C-C_n]$ transformations will lead to
two different graphs if $k\ne l$ and $k,l\ne 0$. Subsequent GC transformations are implemented as well, i.e. $GC_{k_1,l_1} \circ GC_{k_2,l_2} \circ GC_{k_3,l_3}\ldots$ with $GC_{k_1,l_1} \circ GC_{k_2,l_2} = GC_{k_1k_2-l_1l_2,l_1k_2+(k_1+l_1)l_2}$.
...
\item[{itop}] If \paramname{itop} = 1, after creating the adjacency matrix skips directly to the topological analysis. Good for very large fullerenes, e.g. Goldberg-Coxeter transforms. Note, this skips any other actions including transformations and it will not create print out cartesian coordinates (it prints zeros instead). If \paramname{itop} = 2 in addition writes out the connectivity vector to external file \filename{ic3file} (formatted), which can be used as further input.
\end{description}
...
\paramname{ICart, rspi, jumps, IV1, IV2, IV3, isonum, leap, IGCtrans, kGC, ISW, KE, IYF, ISW, mirror, IPRC, R5, R6, ScaleRad, nanotube}
...
If \paramname{ICart} = 4, 5: Goldberg-Coxeter transformation of \C{20} is used, i.e. $GC_{k,l}(\C{20})$. This is computationally cheaper than for any other fullerene.\\
...
\item[{ICGtrans}] 
If \paramname{IGCtrans} = 1 the initial fullerene structure is subjected to a subsequent Goldberg-Coxeter transformation $GC_{k,l}[\C{N_A}]$ (Default: 0). In this case \paramname{kGC} in the namelist input has to be specified (which can be negative). It also allows for subsequent GC transformations of a fullerene, i.e. $GC_{k_1,l_1}\circ GC_{k_2,l_2} \circ GC_{k_3,l_3}(\C{N_A})$ require the input \paramname{kGC}=$k_1,l_1,k_2,l_2,k_3,l_3$. A maximum of up to 10 transformations is allowed.
For $l_1=0$ (halma transformation), $k_1=1$ and $l_1=1$ (leapfrog transformation), and for the general Goldberg-Coxeter transformation of \C{20} more efficient and faster algorithms are implemented compared to the general case. 
\item[ISW] 
