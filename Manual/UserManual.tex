...
eigenvector (AME) method \cite{Atlas}. One can also construct the $n$-th leapfrog $GC_{1,1}^n$[C$_{n_v}$] fullerene or a halma fullerene 
...
minimization with analytical gradients for the (extended) Wu force-field \cite{Wu87} is implemented in the current version, providing 
...
AME or 3D-TE algorithm. We recommend CYLview, it is more robust and works for large fullerenes up to about 1000 atoms. 
...
For this an external file is written out called {\bf graph2D.dat} containing all the information needed for plotting a 2D graph.\\
...
3) Geometry optimization using other force fields; 4) Frequency calculations from the force-field 
...
of genus 1; 9) To symmetrize coordinates to point group symmetry; 10) A restart option for subroutine Hamilton; 11) Use of ``House of Graphs'' 
...
The most time consuming steps in the program are the diagonalization of the adjacency matrix ($O(n^3)$)needed for the AME algorithm and the H\"uckel analysis, 
the force-field optimization for very large fullerenes ($>$ 2000 atoms), the determination of the number of Hamiltonian cycles ($>$ 80 atoms) (this is a NP-complete problem), the creation of a list of ring-spiral pentagon indices for all possible isomers with a given vertex count $n_v$ ($>$ 80 atoms), 
...
Most of these codes are all freely available. For this purpose a file is written out to cylview.xyz to be used as an input file 
for these molecular visualization programs. The program sets the barycenter of the fullerene 
...
index are pronted out. For details see a book on graph theory.
...
is in currently in preparation. 
...
against a second algorithm for various fullerenes. Note, the number gives all distinct Hamiltonian cycles and left-right cycles counted as the 
...
dihedral angles are fixed, the molecule might distort away from convexity. In such a case a Coulomb repulsive potential can
...
Information from the subroutine {\bf RingC} can be used to perform Stone-Wales transformations \cite{Stone86} (subroutine {\bf StoneWalesTrans}) , 
...
and {\it c384.inp}. The two structures obtained with the program PYMOL are shown in Figure \ref{pic:C380andC384}.
...
Subroutine {\bf SpiralSearch}) uses ring-spiral algorithm of Fowler and Manolopoulus for the search of a ring spiral if, for the input,
cartesian coordinates were chosen or the initial fullerene structure was modified. 
It produces the canonical ring spiral pentagon indices if the fullerene is spiral and fails if it is non-spiral (as for the two examples shown in 
...
3) The tesselation into trigonal pyramids from the barycenter by summing over all
tetrahedrons spanned by the three vectors (identical to algorithm 1 and 2 if
...
\&Graph options: {\bf ISchlegel, ISO1, ISO2, ISO3, IFS, PS, Scale, ScalePPG, graphname, texname} (Default: 0 for all values except for {\bf graphname}). We recommend {\bf IG} = 1, 2, 4 or 7.\\
...
carefully chosen. The picture produced gives a good idea if {\bf PS} was correctly chosen or not. Note that the ring centers get
...
If {\bf IFS} = 1: A tex file named {\bf texname} is produced which contains the 2D fullerene graph (Default: 0).\\
If {\bf IFS} = 2: A data file named {\bf graphname} is produced which contains all the information 
for producing a 2D fullerene graph with a plotting program.\\
If {\bf IFS} = 3: Both files are produced.\\
If {\bf ndual} = 1: The dual graph is drawn as well in texfile{\bf texname}. (Default: 0).\\
{\bf graphname} = name. A name of up to 16 characters must be chosen, the file is then named name.dat (Default: graph2D.dat).\\
{\bf texname} = name. A name of up to 16 characters must be chosen, the file is then named name.tex (Default: graph2D.tex).\\
The different algorithms for drawing 2D fullerene graphs are shown in Figure \ref{pic:Graph2D}.
...
\bibitem{Tutte} W. T. Tutte, ``How to Draw a Graph", {\it Proc. London Math. Soc.} 13, 743Ð767 (1963).
...
{\it Linear Algebra Appl.} {\bf 356}, 37Ð56 (2002).
...
\bibitem{pisanski95} T. Pisanski, B. Plestenjak, and A. Graovac, ``Nice Program and its Application, {\it Croat. Chem. Acta} {\bf 68}, 283Ð292 (1995).
