...
(input file \filename{database.inp}). The vertex connections (edges) can also be used as an input, for an example see input file \filename{c20edge.inp}.
The program also prints internal coordinates in form of a Z-Matrix (distances, angles and dihedrals).
...
If \paramname{ICart} = 6 or 7 input vertex connectivities directly. If \paramname{isonum} $\neq 0$, then the isomer number from the isomer list contained in the
``House of Graphs'' database is taken.
\item[{IPRC}]
