...
If \paramname{ICart} = 4, 5: Goldberg-Coxeter transformation of \C{20} is used, i.e. $GC_{k,l}(\C{20})$ with $k \geq l$ and $k > 0$. This is computationally cheaper than for any other fullerene.\\
...
\item[{ICGtrans}] 
If \paramname{IGCtrans} = 1 the initial fullerene structure is subjected to a subsequent Goldberg-Coxeter transformation $GC_{k,l}[\C{N_A}]$ (Default: 0). In this case \paramname{kGC} and \paramname{lGC} in the namelist input have to be specified (which can be negative).
For $l=0$ (halma transformation), $k=1$ and $l=1$ (leapfrog transformation), and for the general Goldberg-Coxeter transformation of \C{20} $(k\ge 1, l\ge 0)$ more efficient and faster algorithms are implemented compared to the general case. 
\item[ISW] 
