...
\item[{iwext}] Flag for producing input file for \program{CYLview}, \program{Avogadro} or other plotting programs in standard formats (Default: 0)
  ...
  i.e. \filename{filename-3D1.xyz}, \filename{filename-3D2.xyz} etc.\\
...
\item[{irext}] Flag for reading external input file which contains cartesian coordinates or connectivities (Default: 0). Note this makes the input of \paramname{ICart} obsolete if set non-zero.
  If \paramname{irext} = 1: Read \filename{.xyz} from file specified by \paramname{filename}. The final name is chosen as \filename{filename-3D.xyz} (Default: \filename{Fullerene-3D.xyz}). 
  Another \filename{.xyz} file is also produced and named \filename{filename-3D1new.xyz} if \paramname{iwext} = 1 is specified.\\
  If \paramname{irext} = 2: Read \filename{.cc1} from file specified by \paramname{filename}. The final name is chosen as \filename{filename-3D.cc1} (Default: \filename{Fullerene-3D.cc1}). 
  Another \filename{.cc1} file is also produced and named \filename{filename-3Dnew.cc1} if \paramname{iwext} = 2 is specified.
  ...
  If \paramname{irext} = 3: Read \filename{.mol2} from file specified by \paramname{filename}. The final name is chosen as \filename{filename-3D.mol2} (Default: \filename{Fullerene-3D.mol2}). 
  Another \filename{.mol2} file is also produced and named \filename{filename-3Dnew.mol2} if \paramname{iwext} = 1 is specified.
