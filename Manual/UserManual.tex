...
like CYLview \cite{CYLview}, Avogadro \cite{Avogadro}, Jmol \cite{JMol}, or Pymol \cite{Pymol}.  For using these programs 
...
The numbering scheme for IPR isomers only ($N_{isomers}^{IPR}= 1,\dots, n_{max}$) can also be used up to \C{120}, see input file {\it database.inp}.
...
The general Goldberg-Coxeter construction $GC_{k,l}[G_{0}]$ works in the following way:
($i$) consider a cubic planar graph $G_{0}$ and take its dual (faces become vertices and
...
The Wu force-field optimization may in some cases distort the fullerene from the ideal point group symmetry. Moreover, as no
...
\marc{actually, the structures were not obtained by pymol ...}
...
There are 5 such tetrahedrons in a pentagon and 6 in a hexagon.  The barycenter CM has been set to the orign in a previous step.
...
\begin{equation}
 q_{\textrm IPQ} =36\pi \frac{V^{2} }{S^{3}}\quad\text{with}q_{\textrm IPQ}\quad\in [0,1]
 \label{IPQ}
 \end{equation}
...
In the case of nonplanar 5- or 6-rings there is no unique definition for the 
volume of a fullerene, except for the convex hull. There is no reason, however, why any other definition should be preferred over the 
fast tesselation algorithm employed in this program.
...
\begin{equation}
	\label{GrindEQ__20_}
  	d_{\mathrm{max}} = \max_{j,j_P}\left\{ D_{ij_P} \right\}\quad\text{and}d_{ip} = \min_{j_P}\left\{ D_{ij_P} \right\}
\end{equation}
...
A converged energy much greater than zero implies that the set distances and angles in the Wu force field cannot be
reached for all atoms and rings.\\
\marc{we need a number because any number is much greater than zero}
...
computationally too demanding. The longest file for our database ran for 3~months on a single processor.
The directory database needs to be in the same directory as ``source'' or ``libgraph''.
\marc{we need to either specify the processor or not mention it at all}
