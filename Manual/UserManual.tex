...
their Fortran subroutines which we modified and implemented in this program.
...
which needed to be addressed, and it was therefore decided extend version 3 to a general purpose program package and make it available
...
If you use the fullerene database (which is recommended for time savings), the directory \filename{database} needs 
to be created and put into the main directory where the \filename{source} and \filename{libgraph} directories are located. \filename{database} contains
...
(input file \filename{database.inp}). Information of vertex connections (edges) can also be used (input files \filename{c20edge.inp} and \filename{c60edge.inp}).
...
of this algorithm is that the structure could be far off from a force-field optimized global minimum for a fullerene and the
...
The numbering scheme for IPR isomers only ($N_\mathrm{isomers}^\mathrm{IPR}= 1,\dots, n_\mathrm{max}$) can also be used up to \C{160}, see input file \filename{database.inp}.
...
There are several other topological indicators of which the Estrada index and mean topological distance, the Wiener index, the hyper-Wiener
...
matrix $A(G)$, $\{ x_i, i=1,...,n_v \} \in [-3,3]$ of a graph $G$ (in our case a regular fullerene graph) \cite{Estrada,Doslic2005},
\begin{equation}
  \label{Estrada}
...
the indices already defined here. Further listed are two topological efficiency parameters $\rho$ and $\rho E$ defined by Vukicevic, Ori and co-workers \cite{Vukicevic,Ori},
\begin{equation}
  \label{rho}
...
\end{equation}
with $r_i - r_0$ being the distance between vertex $i$ and the barycenter. Another method to influence convergence into the global
...
e.g. barrels instead of nanotubes. A more sophisticated force field using dihedral angles is implemented as well (extended Wu force field),
which enhances planarity for areas of connected hexagons.

There is an extension of the Wu force field which takes three types of bonds (adjacent to 0, 1 or 2 pentagons),
...
If \paramname{ICart} = 2 or 3 and \paramname{isonum} $\neq 0$, then pentagon indices are taken from the isomer list contained in a database (see below).\\
If \paramname{ICart} = 6 or 7 input vertex connectivities (edges) directly. If \paramname{isonum} $\neq 0$, then the isomer number from the isomer list contained in the
``House of Graphs'' database is taken. For the vertex connectivities input as series of cards is required each with 4 integers: IV1, IC1, IC2 IC3, i.e. 1 3 6 4 gives
the following edges: 1-3, 1-6, and 1-4. Zeros for IC2 and IC3 can be used, i.e. 1 3 6 0 gives 1-3 and 1-6, and 1 3 0 0 just gives 1-3 as an edge. For the last card
in the input stream, IV1 needs to be set to zero. For an example see input file \filename{c20edge.inp}.  It is legal but not necassary to define edges twice.

When importing graphs from ``House of Graphs'' the isomers are numbered starting at 1 (Although, internally we start counting at 0.).
\item[{IPRC}]
...
The different algorithms for drawing 2D fullerene graphs are shown in Figure \ref{Schlegel}. Note that for very large
...
up to \C{120} for general and up to \C{170} for IPR isomers. The database should be copied into
...
intention to extend the isomer list beyond \C{120}/\C{170} (without Hamiltonian cycles) using the ``House of Graphs'' database . 
...
Note that the maximum number of vertices in the database is et in the config.f file (LimitALL and LimitIPR).
