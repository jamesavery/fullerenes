...
int main(int ac, char **argv)
{
  ...
  mkdir(output_dir.c_str(),0777);
    ...
    while(more_to_generate){
        ...
        std::cout << poly_map.size()<< endl;
    }
    std::vector<float> X0(num_fullerenes.find(N)->second * N*3);

    for (int i = 0; i < num_fullerenes.find(N)->second; ++i){
        for (int j = 0; j < N; ++j){
            for (int k = 0; k < 3 ; ++k) {
                X0[i*N*3 + j*3 + k] = poly_map.at(i).points[j][k];
        }
    }}

    ofstream file("isomerspace_" + to_string(N) + "_X0.bin", ios::binary);
    file.write((char*)X0.data(),X0.size()*sizeof(float));
