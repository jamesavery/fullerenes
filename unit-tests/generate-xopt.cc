...
int main(int ac, char **argv)
{
  ...
  mkdir(output_dir.c_str(),0777);
    ...
    size_t X0_kernel_size   = X0_kernel.get_batch_capacity();
    ...
    while(more_to_do){
        ...
        while (!ff_kernel.output_queue.empty())
        {
            ...
            auto& [ID, P] = ff_kernel.output_queue.front();
            poly_map.insert({ID,P});
            //Polyhedron::to_file(P,output_dir+"/P-C"+to_string(N)+"-"+to_string(ID)+".mol2");
        ...
        if ((ff_kernel.get_converged_count() + ff_kernel.get_failed_count()) > 160000){break;} 
        //break;

    }
    std::vector<float> X(num_fullerenes.find(N)->second * N*3);
    std::vector<float> xys(num_fullerenes.find(N)->second * N*2);
    std::vector<int> neighbours(num_fullerenes.find(N)->second * N*3);
    
    

    for (int i = 0; i < num_fullerenes.find(N)->second; ++i){
        for (int j = 0; j < N; ++j){
            xys[i*N*2 + j*2 + 0] = poly_map.at(i).layout2d[j].first;    
            xys[i*N*2 + j*2 + 1] = poly_map.at(i).layout2d[j].second;    
            
            for (int k = 0; k < 3 ; ++k) {
                X[i*N*3 + j*3 + k] = poly_map.at(i).points[j][k];
                neighbours[i*N*3 + j*3 +k] = poly_map.at(i).neighbours[j][k];
        }
    }}

    ofstream file("isomerspace_" + to_string(N) + "_X_opt.bin", ios::binary);
    ofstream file1("isomerspace_"+ to_string(N) + "_xys.bin", ios::binary);
    ofstream file2("isomerspace_" + to_string(N) + "_neighbours.bin", ios::binary);
    file.write((char*)X.data(),X.size()*sizeof(float));
    file1.write((char*)xys.data(),xys.size()*sizeof(float));
    file2.write((char*)neighbours.data(),neighbours.size()*sizeof(int));
    file.close();
    file1.close();
    file2.close();
    return 0;
