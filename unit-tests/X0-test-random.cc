...
int main(int argc, char** argv){
    ...
    for (size_t i = N_start; i < N_end + 1; i+=2)
    {
        ...
        device_io::IsomerQueue test_queue(i);
        device_io::IsomerQueue test_queue_ex(i);
        device_io::IsomerQueue validation_queue(i);
        ...
        device_io::copy(h_test, d_test); device_io::copy(h_validation, d_validation); device_io::copy(h_test_ex, d_test_ex);
        device_io::sort(h_test); device_io::sort(h_validation); device_io::sort(h_test_ex);
        //std::cout << h_test;
        //std::cout << h_validation;
        std::vector<float> rdiffs(max_samples);
        std::vector<float> adiffs(max_samples);
        std::vector<float> rdiffs_ex(max_samples);
        std::vector<float> adiffs_ex(max_samples);
        for (size_t j = 0; j < batch_size; j++)
        {
            auto [a, adiff, rdiff] = device_io::compare_isomer_arrays(h_test.X + 3*i*j, h_validation.X + 3*i*j, 1, i);
            rdiffs[j] = rdiff;
            adiffs[j] = adiff;
            auto [ex, adiff_ex, rdiff_ex] = device_io::compare_isomer_arrays(h_test_ex.X + 3*i*j, h_validation.X + 3*i*j, 1, i);
