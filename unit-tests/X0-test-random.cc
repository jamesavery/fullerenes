...
int main(int argc, char** argv){
    ...
    ofstream rel_file("X0_validation_rand_rel_" + to_string(N_start) + "_" + to_string(N_end) + ".txt");
    ofstream abs_file("X0_validation_rand_abs_" + to_string(N_start) + "_" + to_string(N_end) + ".txt");

    ofstream rel_ex_file("X0_validation_rand_ex_rel_" + to_string(N_start) + "_" + to_string(N_end) + ".txt");
    ofstream abs_ex_file("X0_validation_rand:ex_abs_" + to_string(N_start) + "_" + to_string(N_end) + ".txt");
    ...
    for (size_t i = N_start; i < N_end + 1; i+=2)
    {
        ...
        auto batch_size = min(max_samples, (int)available_samples);
        ...
        IsomerBatch h_test_ex(i,batch_size,HOST_BUFFER);
        IsomerBatch h_validation(i,batch_size,HOST_BUFFER);
        IsomerBatch d_test(i,batch_size,DEVICE_BUFFER);
        IsomerBatch d_test_ex(i,batch_size,DEVICE_BUFFER);
        IsomerBatch d_validation(i,batch_size,DEVICE_BUFFER);

        Graph F(i,true);
        FullereneDual FD(F);
        FD.neighbours = neighbours_t(i,vector<node_t>(3)); 
        for (int I= 0; I < batch_size; ++I){
            for(int j = 0; j < i; j++){
                for (int k = 0; k < 3; k++){
                    FD.neighbours[j][k] = input_buffer[i*I*3 + j*3+k];
                }
            }
            test_queue_ex.insert(FD,I, LaunchCtx(), LaunchPolicy::SYNC, false);
            FD.layout2d =  FD.tutte_layout();
            test_queue.insert(FD,I, LaunchCtx(), LaunchPolicy::SYNC, true);

            Polyhedron P(FD);
            P.points = P.zero_order_geometry();
            validation_queue.insert(P,I, LaunchCtx(), LaunchPolicy::SYNC, true);
        }
        test_queue.refill_batch(d_test);                        validation_queue.refill_batch(d_validation); test_queue_ex.refill_batch(d_test_ex);
 
        gpu_kernels::isomerspace_tutte::tutte_layout(d_test_ex);
        gpu_kernels::isomerspace_X0::zero_order_geometry(d_test_ex,4.0); gpu_kernels::isomerspace_X0::zero_order_geometry(d_test,4.0);

        cuda_io::copy(h_test, d_test); cuda_io::copy(h_validation, d_validation); cuda_io::copy(h_test_ex, d_test_ex);
        cuda_io::sort(h_test); cuda_io::sort(h_validation); cuda_io::sort(h_test_ex);
        //std::cout << h_test;
        //std::cout << h_validation;
        std::vector<device_real_t> rdiffs(max_samples);
        std::vector<device_real_t> adiffs(max_samples);
        std::vector<device_real_t> rdiffs_ex(max_samples);
        std::vector<device_real_t> adiffs_ex(max_samples);
        for (size_t j = 0; j < batch_size; j++)
        {
            auto [a, adiff, rdiff] = cuda_io::compare_isomer_arrays(h_test.X + 3*i*j, h_validation.X + 3*i*j, 1, i);
            rdiffs[j] = rdiff;
            adiffs[j] = adiff;
            auto [ex, adiff_ex, rdiff_ex] = cuda_io::compare_isomer_arrays(h_test_ex.X + 3*i*j, h_validation.X + 3*i*j, 1, i);
            rdiffs_ex[j] = rdiff_ex;
            adiffs_ex[j] = adiff_ex;
        }
        ...
        std::cout << "Isomerspace: " << i << " RErr:" << std::reduce(rdiffs_ex.begin(),rdiffs_ex.end())/(device_real_t)batch_size << "\n";
        std::cout << "Isomerspace: " << i << " AErr:" << std::reduce(adiffs.begin(),adiffs.end())/(device_real_t)batch_size << "\n";
        std::cout << "Isomerspace: " << i << " AErr:" << std::reduce(adiffs_ex.begin(),adiffs_ex.end())/(device_real_t)batch_size << "\n";

        rel_file << batch_size << ", ";
        rel_ex_file << batch_size << ", ";
        abs_file << batch_size << ", ";
        abs_ex_file << batch_size << ", ";

        for (size_t j = 0; j < max_samples; j++){
            if (j != max_samples-1) {
                rel_file << rdiffs[j] << ", ";
                rel_ex_file << rdiffs_ex[j] << ", ";
                abs_file << adiffs[j] << ", ";
                abs_ex_file << adiffs_ex[j] << ", ";
            }
            else{
                rel_file << rdiffs[j] << "\n";
                abs_file << adiffs[j] << "\n";
