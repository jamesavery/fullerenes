...
//  }
  ...
  int i = 0; // fix 0 in the first position of the first ring
  for (int j=0; j<12; ++j){
    if(i==j) continue;
    for(int k=0; k<12; ++k){
      if(k==i || k==j) continue;
...
//        cout << i << ", " << j << ", " << k << ", " << l << ", " << m << ", " << n << endl;
            ...
            do {
                if(pd[i][o[0]]==pd[n][o[5]] && pd[i][o[1]]==pd[n][o[0]] && pd[i][o[2]]==pd[n][o[1]] && pd[i][o[3]]==pd[n][o[2]] && pd[i][o[4]]==pd[n][o[3]] && pd[i][o[5]]==pd[n][o[4]] &&
            ...
            } while ( std::next_permutation(o.begin()+1, o.end()) );//leave the first in the first position
          }
        }
      }
    }
