...
int main(int argc, char** argv){
    ...
    ofstream rel_file("tutte_validation_rand_rel_" + to_string(N_start) + "_" + to_string(N_end) + ".txt");
    ofstream abs_file("tutte_validation_rand_abs_" + to_string(N_start) + "_" + to_string(N_end) + ".txt");
    ...
    for (size_t i = N_start; i < N_end + 1; i+=2)
    {
        ...
        {
            auto [a, adiff, rdiff] = device_io::compare_isomer_arrays(h_test.xys + 2*i*j, h_validation.xys + 2*i*j, 1, i);
            rdiffs[j] = rdiff;
            adiffs[j] = adiff;
        }
