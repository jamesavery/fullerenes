...
int main(int ac, char **argv){
    ...
    device_io::sort(d_control, IDS);
    device_io::sort(output_queue.device_batch, IDS);
    ...
    device_io::copy(h_control, d_control);
    output_queue.host_batch.shrink_to_fit();
    device_io::copy(output_queue.host_batch, output_queue.device_batch);
    ...
    {
        auto [n_correct_b,adiff_b, rdiff_b ] = device_io::compare_isomer_arrays(test_bonds.data + i*N*3, control_bonds.data +i*N*3,1, N*3, 1e-4);
        auto [n_correct_a,adiff_a, rdiff_a ] = device_io::compare_isomer_arrays(test_angles.data + i*N*3, control_angles.data +i*N*3,1, N*3, 1e-4);
        auto [n_correct_d,adiff_d, rdiff_d ] = device_io::compare_isomer_arrays(test_dihedrals.data + i*N*3, control_dihedrals.data +i*N*3,1, N*3, 1e-4);
