...
```bash
# Configure (from repo root). Let CMake auto-detect the default toolchain:
mkdir build && cd build
cmake ..
# On macOS (Apple clang), tests/tools that use OpenMP need Homebrew libomp,
# since Apple clang ships without it: brew install libomp, then build/run
# with libomp on the include/lib/rpath search paths. The library itself
# (libfullerenes.dylib) builds with the auto-detected clang as-is.
...
If `libfullerenes.so` aborts with `length_error` during dynamic-load static
init, this has been observed with gcc-14 + `-std=gnu++23` miscompiling the
...
- **Debug toggles via CLI args, NEVER environment variables**: Test drivers and binaries should accept debug / diagnostic options via command-line flags (parsed from `argv`), with a corresponding setter API (e.g., `set_debug(bool)`) in the production code so tests can flip the flag programmatically. Do NOT use environment variables (`FOO=1 ./test_x`). Reason: the permissions system auto-approves stable command shapes; adding/removing an env var prefix changes the shape and forces a manual permission prompt every time the var is toggled. When touching code that has env-var debug knobs, refactor them to CLI flags as part of the change.
- **No `timeout N ./binary` prefix**: Don't wrap test commands in `timeout` from the shell — same permission-shape problem as env vars. Ensure termination by building guards INTO the binary itself: `MAX_STEPS` counters in loops with explicit `abort()` on trip, internal time budgets, signal handlers, or checkpoint-based bounded sub-tasks. On guard trip, print a clear diagnostic naming what hit the limit. If a binary currently lacks termination guards, add them as part of the change.
- **NEVER abandon scoped work without prior discussion**: When the user has explicitly scoped a piece of work (e.g., "build a robust check for iDT surface metric correctness"), do not silently replace it with a different deliverable just because the original is hard. If the chosen approach hits an obstacle, STOP and tell the user: what the goal was, what you've tried, why it's not working, and what alternatives you see — then let the user decide. Pivoting to a smaller / different deliverable on your own is a *much* worse outcome than briefly pausing to discuss: the user has already paid the context-and-attention cost of scoping the original task, and discarding that work without consultation wastes both their effort and your accumulated understanding. This rule binds even when the substitute looks plausible to you and even when you think the original goal is unreachable — present that conclusion to the user; don't enact it. Applies to: replacing a global geodesic distance computation with local invariants, swapping floating-point for integer arithmetic (or vice versa) without checking whether the user wanted exactness, dropping a problem and verifying a different one, narrowing the scope of a verifier, or switching from "compute X" to "check that X is plausible". Default action when stuck: write a 3-bullet message — *original task*, *blocker*, *options* — and wait for direction.
