...
12    continue
     ...
     1 fa,fb,fc,Dist)
      ...
      REAL*8 Dist(3,N)
      fa=f1dimx(N,IOP,ier,ax,pcom,xicom,Dist)
      fb=f1dimx(N,IOP,ier,bx,pcom,xicom,Dist)
      if(fb.gt.fa)then
        dum=ax
        ax=bx
        bx=dum
      ...
      cx=bx+GOLD*(bx-ax)
      fc=f1dimx(N,IOP,ier,cx,pcom,xicom,Dist)
1     if(fb.ge.fc)then
        r=(bx-ax)*(fb-fc)
        q=(bx-cx)*(fb-fa)
        ...
        ulim=bx+GLIMIT*(cx-bx)
        if((bx-u)*(u-cx).gt.0.)then
          fu=f1dimx(N,IOP,ier,u,pcom,xicom,Dist)
          if(fu.lt.fc)then
            ax=bx
            fa=fb
            bx=u
     ...
     1 pcom,xicom,ax,bx,cx,tol,xmin,Dist)
      ...
      x=v
      e=0.
      fx=f1dimx(N,IOP,ier,x,pcom,xicom,Dist)
      fv=fx
      fw=fx
      do 11 iter=1,ITMAX
        xm=0.5*(a+b)
