...
\texttt{DelaunayTriangulation} & intrinsic Delaunay $\Delta$-complex (half-edge DCEL) & copied arrays \\
\texttt{GeoOptions}, \texttt{GeoHeader}, \texttt{GeoType} & compact binary \texttt{.geo} archives (\texttt{GEO-FORMAT.md}) & files \\
\addlinespace
...
for dual in fl.buckygen(60):
    name = dual.name()           # canonical: "C60-[1,7,9,...]-fullerene"
...
\item \texttt{naming.hh} --- the \code{canonical_name} helper, \texttt{C<N>-} followed by
  the fullerene's spiral name (grammar in \texttt{spiral.hh});
