...
template <typename T, typename K, int BlockVectors, int NZ> class LOBPCG_V6_1 {};
template <typename T, typename K, int BlockVectors, int NZ> class LOBPCG_V6_2 {};
template <typename T, typename K, int BlockVectors, int NZ> class LOBPCG_V6_3 {};
template <typename T, typename K, int BlockVectors, int NZ> class LOBPCG_V6_4 {};
...
void CholQR(SyclQueue &ctx, Span<T*> S_ptrs /* Subspace */,
    ...
    static SyclVector<T*> Q_ptrs(batch_size);
    static SyclVector<int> d_info(batch_size);    
    static bool initialized = false;
    if (!initialized){
    ...
    //assert(std::distance(STS_ptrs[0], STS_ptrs[1]) == SN*SN);
    constexpr T alpha = 1.0;
    constexpr T beta = 0.0;
...
void LOBPCG_V6(SyclQueue &ctx, Span<T> A, Span<K> cols, int batch_size, int m, size_t maxiters, bool largest, Span<T> out_eigvects, Span<T> out_eigvals){
    ...
    #endif
    const T tol = sycl::sqrt(std::numeric_limits<T>::epsilon()) * 1e-1 * T(m);
    ...
    ctx -> submit([&](sycl::handler& h ){
        ...
        h.parallel_for<LOBPCG_V6_1<T,K,BlockVectors,NZ>>(nd_range<1>(sycl::range{size_t(batch_size*m)}, sycl::range{size_t(m)}), [=](sycl::nd_item<1> item){
            auto tid = item.get_local_linear_id();
    ...
    //SVQB<T, BlockVectors>(ctx, S, StAS, Diag, U, m, SubspaceStride, batch_size);
    //cudaDeviceSynchronize();
    //cudaMemcpy(S.data(), U.data(), sizeof(T) * batch_size * BlockVectors * 3 * m, cudaMemcpyDeviceToDevice);
    cudaDeviceSynchronize();
    for(int i = 0; i < batch_size; i++){
        S_ptrs[i] = S.data() + i * (BlockVectors*3) * m + 2*BlockVectors*m; //R block
    }
    //Compute AX
    ...
    ctx -> submit([&](sycl::handler& h){
        ...
        h.parallel_for<LOBPCG_V6_2<T,K,BlockVectors,NZ>>(nd_range<1>(sycl::range{size_t(batch_size*m)}, sycl::range{size_t(m)}), [=](sycl::nd_item<1> item){
            auto tid = item.get_local_linear_id();
            sycl::group<1> cta = item.get_group();
            auto bid = item.get_group_linear_id();
    ...
    auto iteration = [&](SyclQueue &ctx, Span<T> A, Span<K> cols, int batch_size, int m, size_t iter, bool restart){
        ...
        ctx -> submit([&](sycl::handler& h){
            ...
            h.parallel_for<LOBPCG_V6_3<T,K,BlockVectors,NZ>>(nd_range<1>(sycl::range{size_t(batch_size*m)}, sycl::range{size_t(m)}), [=](sycl::nd_item<1> item){
                auto tid = item.get_local_linear_id();
                auto cta = item.get_group();
                auto bid = item.get_group_linear_id();
        ...
        if (restart){
            ExternalOrthogonalization<T, BlockVectors, BlockVectors>(ctx, S.subspan(BlockVectors*m*2), S, StAS, m, m*3*BlockVectors, m*3*BlockVectors, batch_size);
            //SVQB<T, BlockVectors>(ctx, S.subspan(BlockVectors*m*2), StAS, Diag, U, m, BlockVectors*3*m, batch_size);
            CholQR<T, BlockVectors>(ctx, S_ptrs, STS_ptrs, m, batch_size);
        } else {
            ExternalOrthogonalization<T, 2*BlockVectors, BlockVectors>(ctx, S.subspan(BlockVectors*m*2), S, StAS, m, m*3*BlockVectors, m*3*BlockVectors, batch_size);
...
template void LOBPCG_V6<float, uint16_t, 3, 30>(SyclQueue &ctx, Span<float> A, Span<uint16_t> cols, int batch_size, int m, size_t maxiters, bool largest, Span<float> eigvects, Span<float> eigvals);
template void LOBPCG_V6<float, uint16_t, 21, 30>(SyclQueue &ctx, Span<float> A, Span<uint16_t> cols, int batch_size, int m, size_t maxiters, bool largest, Span<float> eigvects, Span<float> eigvals);