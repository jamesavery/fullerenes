...
template <typename T, typename K, int BlockVectors, int NZ> class LOBPCG_V5_1 {};
template <typename T, typename K, int BlockVectors, int NZ> class LOBPCG_V5_2 {};
template <typename T, typename K, int BlockVectors, int NZ> class LOBPCG_V5_3 {};
template <typename T, typename K, int BlockVectors, int NZ> class LOBPCG_V5_4 {};
...
void LOBPCG_V5(SyclQueue &ctx, Span<T> A, Span<K> cols, int batch_size, int m, size_t maxiters, bool largest, Span<T> out_eigvects, Span<T> out_eigvals){
    ...
    ctx -> submit([&](sycl::handler& h ){
        ...
        h.parallel_for<LOBPCG_V5_1<T,K,BlockVectors,NZ>>(nd_range<1>(sycl::range{size_t(batch_size*m)}, sycl::range{size_t(m)}), [=](sycl::nd_item<1> item){
            auto tid = item.get_local_linear_id();
    ...
    ctx -> submit([&](sycl::handler& h){
        ...
        h.parallel_for<LOBPCG_V5_2<T,K,BlockVectors,NZ>>(nd_range<1>(sycl::range{size_t(batch_size*m)}, sycl::range{size_t(m)}), [=](sycl::nd_item<1> item){
            auto tid = item.get_local_linear_id();
            sycl::group<1> cta = item.get_group();
            auto bid = item.get_group_linear_id();
    ...
    auto iteration = [&](SyclQueue &ctx, Span<T> A, Span<K> cols, int batch_size, int m, size_t iter, bool restart){
        ...
        ctx -> submit([&](sycl::handler& h){
            ...
            h.parallel_for<LOBPCG_V5_3<T,K,BlockVectors,NZ>>(nd_range<1>(sycl::range{size_t(batch_size*m)}, sycl::range{size_t(m)}), [=](sycl::nd_item<1> item){
                auto tid = item.get_local_linear_id();
                auto cta = item.get_group();
                auto bid = item.get_group_linear_id();
        ...
        if (restart){
            ...
            CholQR<T, BlockVectors*1>(ctx, S.subspan(m*BlockVectors*2), StAS, R_inv, U, m, BlockVectors*3*m, batch_size);
        } else {
            ...
            //SVQB<T, BlockVectors*3>(ctx, S, StAS, Diag, U, m, BlockVectors*3*m, batch_size);
            CholQR<T, BlockVectors>(ctx, S.subspan(m*BlockVectors*2), StAS, R_inv, U, m, BlockVectors*3*m, batch_size);
        }
        ...
        ctx -> submit([&](sycl::handler& h){
            ...
            h.parallel_for(nd_range<1>(sycl::range{size_t(batch_size*m)}, sycl::range{size_t(m)}), [=](sycl::nd_item<1> item){
                ...
                auto block_dst = dst.subspan((bid*3 + (restart ? 1 : 2)) * m * BlockVectors, m * BlockVectors/*  * (restart ? 1 : 2) */);
        ...
        print_matrix(R_inv.subspan(BlockVectors*3*3*BlockVectors), BlockVectors*3 , BlockVectors*3 , false);
        //ComputeGramMatrix<T,BlockVectors*3>(ctx, S, R_inv, m, BlockVectors*m*3, batch_size);
...
template void LOBPCG_V5<float, uint16_t, 3, 30>(SyclQueue &ctx, Span<float> A, Span<uint16_t> cols, int batch_size, int m, size_t maxiters, bool largest, Span<float> eigvects, Span<float> eigvals);
template void LOBPCG_V5<float, uint16_t, 21, 30>(SyclQueue &ctx, Span<float> A, Span<uint16_t> cols, int batch_size, int m, size_t maxiters, bool largest, Span<float> eigvects, Span<float> eigvals);