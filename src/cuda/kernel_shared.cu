...
struct ArcData{
    ...
    INLINE real_t bond_energy(const Constants& c) const {return (real_t)0.5 *get(c.f_bond,j) *harmonic_energy(bond(),get(c.r0,j));}
    INLINE real_t bend_energy(const Constants& c) const {return get(c.f_inner_angle,j)* harmonic_energy(angle(),get(c.angle0,j));}
    INLINE real_t dihedral_energy(const Constants& c) const {return get(c.f_inner_dihedral,j)* harmonic_energy(dihedral(),get(c.inner_dih0,j));}
...
__global__ void FullereneProperties(DevicePointers p){
    ...
    BookkeepingData bookit = BookkeepingData(&p.neighbours[3*(offset)],&p.face_right[3*(offset)],&p.next_on_face[3*(offset)],&p.prev_on_face[3*(offset)]);
    Constants constants = compute_constants(bookit,threadIdx.x);
