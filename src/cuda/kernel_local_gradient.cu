...
__device__ void golden_section_search(coord3d* X, coord3d& direction, coord3d& new_direction,coord3d* X1, coord3d* X2, real_t* reduction_array, real_t a, real_t b, const node_t node_id, const node_t N, const BookkeepingData &dat){
    ...
    cg::sync(block);
    new_direction = -gradient(X1,node_id,dat);
}

__global__ void conjugate_gradient(coord3d* d_X, coord3d* d_X_temp, coord3d* d_X1, coord3d* d_X2, node_t* d_neighbours, node_t* d_next_on_face, node_t* d_prev_on_face, uint8_t* d_face_right, size_t N, size_t MaxIter){
    ...
    direction = gradient(X, node_id ,local_bookkeeping);
    ...
    {   
        ...
        reduction_array[threadIdx.x] = dot(delta_x0, delta_x0); reduction(reduction_array); r0_norm = reduction_array[0];
        cg::sync(block);
        reduction_array[threadIdx.x] = dot(delta_x1, (delta_x1 - delta_x0)); reduction(reduction_array); beta = reduction_array[0] / r0_norm;
        cg::sync(block);
        ...
        cg::sync(block);
        reduction_array[threadIdx.x] = dot(direction,direction); reduction(reduction_array); direction_norm = sqrtf(reduction_array[0]);
        cg::sync(block);
        reduction_array[threadIdx.x] = dot(delta_x1,delta_x1); reduction(reduction_array); dnorm = sqrtf(reduction_array[0]);
        cg::sync(block);
        //Normalize gradient.
