...
__global__ void conjugate_gradient(coord3d* d_X, coord3d* d_X_temp, coord3d* d_X1, coord3d* d_X2, node_t* d_neighbours, node_t* d_next_on_face, node_t* d_prev_on_face, uint8_t* d_face_right, size_t N){
    ...
    {   
        ...
        reduction_array[threadIdx.x] = dot(delta_x0, delta_x0); reduction(reduction_array, N); r0_norm = reduction_array[0];
        cg::sync(block);
        reduction_array[threadIdx.x] = dot(delta_x1, (delta_x1 - delta_x0)); reduction(reduction_array, N); beta = reduction_array[0] / r0_norm;
        cg::sync(block);
        ...
        cg::sync(block);
        reduction_array[threadIdx.x] = dot(direction,direction); reduction(reduction_array, N); direction_norm = sqrtf(reduction_array[0]);
        cg::sync(block);
        reduction_array[threadIdx.x] = dot(delta_x1,delta_x1); reduction(reduction_array, N); dnorm = sqrtf(reduction_array[0]);
        cg::sync(block);
        //Normalize gradient.
