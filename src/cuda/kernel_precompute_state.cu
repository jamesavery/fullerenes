...
__device__ real_t energy(const coord3d* __restrict__ X, const node_t node_id, const BookkeepingData &dat, const Constants &constants, real_t* __restrict__ reduction_array, const size_t N) {
...
__global__ void conjugate_gradient(coord3d* d_X, coord3d* d_X_temp, coord3d* d_X1, coord3d* d_X2, node_t* d_neighbours, node_t* d_next_on_face, node_t* d_prev_on_face, uint8_t* d_face_right, size_t N){
    ...
    BookkeepingData shared_bookkeeping = BookkeepingData(&d_neighbours[3*offset],&d_face_right[3*offset],&d_next_on_face[3*offset],&d_prev_on_face[3*offset]);
    Constants constants = compute_constants(shared_bookkeeping, node_id);
    ...
    {   
        ...
        cg::sync(grid);
        golden_section_search(X, direction, delta_x1, X_temp, X2,reduction_array, 0, 1, node_id, N, bookkeeping, constants);
