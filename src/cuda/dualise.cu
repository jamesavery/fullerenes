...
template cudaError_t dualise<GPU>(IsomerBatch<GPU>& B, const LaunchCtx& ctx, const LaunchPolicy policy);
template cudaError_t dualise_2<GPU>(IsomerBatch<GPU>& B, const LaunchCtx& ctx, const LaunchPolicy policy);
...
void cubic_layout_(IsomerBatch<U> B){
    ...
    device_node2 edge_b = FD.get_cannonical_triangle_arc(v, u); B.cubic_neighbours[isomer_idx*B.n_atoms*3 + thid*3 + 0] = triangle_numbers[edge_b[0] * 6 + FD.dedge_ix(edge_b[0], edge_b[1])];
    device_node2 edge_c = FD.get_cannonical_triangle_arc(w, v); B.cubic_neighbours[isomer_idx*B.n_atoms*3 + thid*3 + 1] = triangle_numbers[edge_c[0] * 6 + FD.dedge_ix(edge_c[0], edge_c[1])];
    device_node2 edge_d = FD.get_cannonical_triangle_arc(u, w); B.cubic_neighbours[isomer_idx*B.n_atoms*3 + thid*3 + 2] = triangle_numbers[edge_d[0] * 6 + FD.dedge_ix(edge_d[0], edge_d[1])];
 ...
 */
    ...
    for (auto tix = thid; tix < B.n_atoms; tix += blockDim.x){
        ...
        device_node_t w = FD.next(u,v);
        device_node2 edge_b = FD.get_cannonical_triangle_arc(v, u); B.cubic_neighbours[isomer_idx*B.n_atoms*3 + tix*3 + 0] = triangle_numbers[edge_b[0] * 6 + FD.dedge_ix(edge_b[0], edge_b[1])];
        device_node2 edge_c = FD.get_cannonical_triangle_arc(w, v); B.cubic_neighbours[isomer_idx*B.n_atoms*3 + tix*3 + 1] = triangle_numbers[edge_c[0] * 6 + FD.dedge_ix(edge_c[0], edge_c[1])];
        device_node2 edge_d = FD.get_cannonical_triangle_arc(u, w); B.cubic_neighbours[isomer_idx*B.n_atoms*3 + tix*3 + 2] = triangle_numbers[edge_d[0] * 6 + FD.dedge_ix(edge_d[0], edge_d[1])];
    }
...
int optimal_batch_size(const int N, const int device_id) {
    ...
    static LaunchDims dims((void*)cubic_layout_<GPU>, N, smem);
    dims.update_dims((void*)cubic_layout_<GPU>, N, smem);
...
int optimal_batch_size_2(const int N, const int device_id) {
    ...
    static LaunchDims dims((void*)dualise_2_<GPU>, N, smem);
    dims.update_dims((void*)dualise_2_<GPU>, N, smem);
    return dims.get_grid().x;
}
template <Device U>
...
cudaError_t dualise_2(IsomerBatch<U>& B, const LaunchCtx& ctx, const LaunchPolicy policy){
    ...
    static LaunchDims dims((void*)dualise_2_<GPU>, threads_rounded_to_next_multiple_of_32, smem, B.isomer_capacity);
    dims.update_dims((void*)dualise_2_<GPU>, threads_rounded_to_next_multiple_of_32, smem, B.isomer_capacity);
    cudaError_t error;
    void* kargs[]{(void*)&B};
    cudaEventRecord(start[dev], ctx.stream);
    error = safeCudaKernelCall((void*)dualise_2_<GPU>, dims.get_grid(), dims.get_block(), kargs, smem, ctx.stream);  
    cudaEventRecord(stop[dev], ctx.stream);
