...
struct ArcData{
    //124 FLOPs;
    __device__ ArcData(const node_t a, const uint8_t j, const half4* __restrict__ X, const BookkeepingData& bdat){   
        this->j = j;   

        half r_rmp;
        half4 ap, am, ab, ac, ad, mp;
        half4 X_a = X[a]; half4 X_b = X[bdat.neighbours[j]];
        //printf("Index: %d \n", a*3 + j);
        ...
        half4 bp = (X[bdat.next_on_face[j]] - X_b); bp_hat = unit_vector(bp);
        half4 bm = (X[bdat.prev_on_face[j]] - X_b); bm_hat = unit_vector(bm);
    ...
    //3 FLOPs
    INLINE half harmonic_energy(const half p0, const half p) const{
        return (half)0.5*(p-p0)*(p-p0);
    }
    //4 FLOPs
    INLINE half4  harmonic_energy_gradient(const half p0, const half p, const half4 gradp) const{
        return (p-p0)*gradp;     
    }

    //1 FLOP
    INLINE half bond() const {return (half)rab;}

    //5 FLOPs
    INLINE half angle() const {return dot(ab_hat,ac_hat);}
    ...
    //50 FLOPs
    INLINE half dihedral() const 
    { 
        half4 nabc, nbcd; half cos_b, cos_c, r_sin_b, r_sin_c;
        cos_b = dot(ba_hat,bc_hat);  r_sin_b = hrsqrt((half)1.0 - cos_b*cos_b); nabc = cross(ba_hat, bc_hat) * r_sin_b;
        cos_c = dot(-bc_hat,cd_hat); r_sin_c = hrsqrt((half)1.0 - cos_c*cos_c); nbcd = cross(-bc_hat,cd_hat) * r_sin_c;
        return dot(nabc, nbcd);
    }
    
    // Chain rule terms for angle calculation
    //Computes gradient related to bending term. ~24 FLOPs
    INLINE half4 inner_angle_gradient(const Constants<half4>& c) const
    {   
        half cos_angle = angle(); //Inner angle of arcs ab,ac.
        half4 grad = cos_angle * (ab_hat * r_rab + ac_hat * r_rac) - ab_hat * r_rac - ac_hat* r_rab; //Derivative of inner angle: Eq. 21. 
    ...
    INLINE half4 outer_angle_gradient_m(const Constants<half4>& c) const
    {
        half cos_angle = -dot(ab_hat, bm_hat); //Compute outer angle. ab,bm
        half4 grad = (bm_hat + ab_hat * cos_angle) * r_rab; //Derivative of outer angles Eq. 30. Buster Thesis
    ...
    INLINE half4 outer_angle_gradient_p(const Constants<half4>& c) const
    {
        half cos_angle = -dot(ab_hat, bp_hat); //Compute outer angle. ab,bp
        half4 grad = (bp_hat + ab_hat * cos_angle) * r_rab; //Derivative of outer angles Eq. 28. Buster Thesis
    ...
    INLINE half4 inner_dihedral_gradient(const Constants<half4>& c) const
    {
        half4 nabc, nbcd; half cos_b, cos_c, r_sin_b, r_sin_c;
        cos_b = dot(ba_hat,bc_hat); r_sin_b = hrsqrt((half)1.0 - cos_b*cos_b); nabc = cross(ba_hat, bc_hat) * r_sin_b;
        cos_c = dot(-bc_hat,cd_hat); r_sin_c = hrsqrt((half)1.0 - cos_c*cos_c); nbcd = cross(-bc_hat,cd_hat) * r_sin_c;

        half cos_beta = dot(nabc, nbcd); //Inner dihedral angle from planes abc,bcd.
        half cot_b = cos_b * r_sin_b * r_sin_b; //cos(b)/sin(b)^2

        //Derivative w.r.t. inner dihedral angle F and G in Eq. 26
        half4 grad = cross(bc_hat, nbcd) * r_sin_b * r_rab - ba_hat * cos_beta * r_rab + (cot_b * cos_beta * r_rab) * (bc_hat - ba_hat * cos_b);
    ...
    INLINE half4 outer_a_dihedral_gradient(const Constants<half4>& c) const
    {
        half4 nbam_hat, namp_hat; half cos_a, cos_m, r_sin_a, r_sin_m;

        cos_a = dot(ab_hat,am_hat); r_sin_a = hrsqrt((half)1.0 - cos_a*cos_a); nbam_hat = cross(ab_hat,am_hat) * r_sin_a;
        cos_m = dot(-am_hat,mp_hat); r_sin_m = hrsqrt((half)1.0 - cos_m*cos_m); namp_hat = cross(-am_hat,mp_hat) * r_sin_m;
        
        half cos_beta = dot(nbam_hat, namp_hat); //Outer Dihedral angle bam, amp
        half cot_a = cos_a * r_sin_a * r_sin_a;
        half cot_m = cos_m * r_sin_m * r_sin_m;

        //Derivative w.r.t. outer dihedral angle, factorized version of Eq. 31.
        half4 grad = cross(mp_hat,nbam_hat)*r_ram*r_sin_m - (cross(namp_hat,ab_hat)*r_ram + cross(am_hat,namp_hat)*r_rab)*r_sin_a +
                        cos_beta*(ab_hat*r_rab + r_ram * ((half)2.0*am_hat + cot_m*(mp_hat+cos_m*am_hat)) - cot_a*(r_ram*(ab_hat - am_hat*cos_a) + r_rab*(am_hat-ab_hat*cos_a)));
    ...
    INLINE half4 outer_m_dihedral_gradient(const Constants<half4>& c) const
    {
        half4 nbmp_hat, nmpa_hat; half cos_m, cos_p, r_sin_m, r_sin_p;
        cos_m = dot(mb_hat,mp_hat);  r_sin_m = hrsqrt((half)1.0 - cos_m*cos_m); nbmp_hat = cross(mb_hat,mp_hat) * r_sin_m;
        cos_p = dot(-mp_hat,pa_hat); r_sin_p = hrsqrt((half)1.0 - cos_p*cos_p); nmpa_hat = cross(-mp_hat,pa_hat) * r_sin_p;
        
        //Cosine to the outer dihedral angle constituted by the planes bmp and mpa
        half cos_beta = dot(nbmp_hat, nmpa_hat); //Outer dihedral angle bmp,mpa.
        half cot_p = cos_p * r_sin_p * r_sin_p;
        
        //Derivative w.r.t. outer dihedral angle, factorized version of Eq. 32.
        half4 grad = r_rap * (cot_p*cos_beta * (-mp_hat - pa_hat*cos_p) - cross(nbmp_hat, mp_hat)*r_sin_p - pa_hat*cos_beta );
    ...
    INLINE half4 outer_p_dihedral_gradient(const Constants<half4>& c) const
    {
        half4 nbpa_hat, npam_hat; half cos_p, cos_a, r_sin_p, r_sin_a;
        cos_a = dot(ap_hat,am_hat);  r_sin_a = hrsqrt((half)1.0 - cos_a*cos_a); npam_hat = cross(ap_hat,am_hat)  * r_sin_a;
        cos_p = dot(pb_hat,-ap_hat); r_sin_p = hrsqrt((half)1.0 - cos_p*cos_p); nbpa_hat = cross(pb_hat,-ap_hat) * r_sin_p;

        half cos_beta = dot(nbpa_hat, npam_hat); //Outer dihedral angle bpa, pam.
        half cot_p = cos_p * r_sin_p * r_sin_p;
        half cot_a = cos_a * r_sin_a * r_sin_a;

        //Derivative w.r.t. outer dihedral angle, factorized version of Eq. 33.
        half4 grad = cross(npam_hat,pb_hat)*r_rap*r_sin_p - (cross(am_hat,nbpa_hat)*r_rap + cross(nbpa_hat,ap_hat)*r_ram)*r_sin_a +
                        cos_beta*(am_hat*r_ram + r_rap * ((half)2.0*ap_hat + cot_p*(pb_hat+cos_p*ap_hat)) - cot_a*(r_rap*(am_hat - ap_hat*cos_a) + r_ram*(ap_hat-am_hat*cos_a)));
    ...
    INLINE half4 bond_length_gradient(const Constants<half4>& c) const { return - get(c.f_bond,j) * harmonic_energy_gradient(get(c.r0,j),bond(),ab_hat);}
    //Sum of angular gradient components.
    INLINE half4 angle_gradient(const Constants<half4>& c) const { return inner_angle_gradient(c) + outer_angle_gradient_p(c) + outer_angle_gradient_m(c);}
    //Sum of inner and outer dihedral gradient components.
    INLINE half4 dihedral_gradient(const Constants<half4>& c) const { return inner_dihedral_gradient(c) + outer_a_dihedral_gradient(c) + outer_m_dihedral_gradient(c) + outer_p_dihedral_gradient(c);}
    //half4 flatness()             const { return ;  }   
    
    INLINE half bond_energy(const Constants<half4>& c) const {return (half)0.5 *get(c.f_bond,j) *harmonic_energy(bond(),get(c.r0,j));}
    INLINE half bend_energy(const Constants<half4>& c) const {return get(c.f_inner_angle,j)* harmonic_energy(angle(),get(c.angle0,j));}
    INLINE half dihedral_energy(const Constants<half4>& c) const {return get(c.f_inner_dihedral,j)* harmonic_energy(dihedral(),get(c.inner_dih0,j));}
    //Harmonic energy contribution from bond stretching, angular bending and dihedral angle bending.
    //71 FLOPs
    INLINE half energy(const Constants<half4>& c) const {return bond_energy(c) + bend_energy(c) + dihedral_energy(c); }
    //Sum of bond, angular and dihedral gradient components.
    INLINE half4 gradient(const Constants<half4>& c) const{return bond_length_gradient(c) +  angle_gradient(c) + dihedral_gradient(c);}
...
__device__ coord3d gradient(const half4* __restrict__ X, const node_t node_id, const BookkeepingData &dat, const Constants<half4> &constants) {
    coord3d grad;

    for (uint8_t j = 0; j < 3; j++ ){
        ArcData arc = ArcData(node_id, j, X, dat);
        grad += fp16_to_coord3d(arc.gradient(constants));
    }
    return grad;
}

__device__ real_t energy(const half4* __restrict__ X, const node_t node_id, const BookkeepingData &dat, const Constants<half4> &constants, real_t* __restrict__ reduction_array, real_t* __restrict__ gdata, const node_t N, bool single_block_fullerenes) {
    real_t arc_energy = (real_t)0.0;

    //(71 + 124) * 3 * N  = 585*N FLOPs
    for (uint8_t j = 0; j < 3; j++ ){
        ArcData arc = ArcData(node_id, j, X, dat);
        arc_energy += __half2float(arc.energy(constants));
    }
...
__device__ void golden_section_search(half4* __restrict__ X, coord3d& direction, coord3d& new_direction,half4* __restrict__ X1, half4* __restrict__ X2, real_t* __restrict__ reduction_array, real_t* __restrict__ gdata, const node_t node_id, const node_t N, const BookkeepingData& dat, const Constants<half4>& constants, cg::thread_group sync_group, bool single_block_fullerenes){
    ...
    X1[node_id] = X[node_id] + coord3d_to_fp16(x1 * direction);
    X2[node_id] = X[node_id] + coord3d_to_fp16(x2 * direction);
    ...
    for (uint8_t i = 0; i < 20; i++){
        ...
        if (( blockIdx.x) == 0)
        {
            sequential_print(direction);
        }
        break;
    }
    //Line search coefficient
    real_t alfa = (a+b)/(real_t)2.0;
    cg::sync(sync_group);
    X1[node_id] = X[node_id] + coord3d_to_fp16(alfa*direction);
    cg::sync(sync_group);
...
__global__ void conjugate_gradient(DevicePointers p, const size_t N, const bool single_block_fullerenes, const size_t MaxIter){
    ...
    half4* X; half4* X1; half4* X2;
    
    if (single_block_fullerenes)
    {
        half4* sX =&reinterpret_cast<half4*>(smem)[(int)ceil(N/2) +1 ];
        half4* sX1 =&reinterpret_cast<half4*>(smem)[(int)ceil(N/2) + 2 + N];
        half4* sX2 =&reinterpret_cast<half4*>(smem)[(int)ceil(N/2) + 3 + 2*N];  
        sX[node_id] = coord3d_to_fp16(reinterpret_cast<coord3d*>(p.X)[offset+node_id]);
    ...
    Constants<half4> constants = compute_constants<half4>(bookit, node_id);
    ...
    {   
        ...
        smem[threadIdx.x] = dot(delta_x0, delta_x0); reduction(smem); r0_norm = smem[0];
        cg::sync(block);
        smem[threadIdx.x] = dot(delta_x1, (delta_x1 - delta_x0)); reduction(smem); beta = smem[0] / r0_norm;
        cg::sync(block);
        ...
        cg::sync(block);
        smem[threadIdx.x] = dot(direction,direction); reduction(smem); direction_norm = sqrtf(smem[0]);
        cg::sync(block);
        smem[threadIdx.x] = dot(delta_x1,delta_x1); reduction(smem);dnorm = sqrtf(smem[0]);
        cg::sync(block);
    ...
    reinterpret_cast<coord3d*>(p.X)[offset + threadIdx.x] = fp16_to_coord3d(X[threadIdx.x]);
}

__global__ void FullereneProperties(DevicePointers p){
    extern __shared__ coord3d sdata[];
    size_t offset = blockIdx.x * blockDim.x;
    half4* X = &reinterpret_cast<half4*>(p.X2)[offset];
    X[threadIdx.x] = coord3d_to_fp16(reinterpret_cast<coord3d*>(p.X)[threadIdx.x + offset]); 

    BookkeepingData bookit = BookkeepingData(&p.neighbours[3*(offset)],&p.face_right[3*(offset)],&p.next_on_face[3*(offset)],&p.prev_on_face[3*(offset)]);
    Constants<half4> constants = compute_constants<half4>(bookit,threadIdx.x);
    ...
    for (uint8_t j = 0; j < 3; j++)
    {
        ...
        ArcBond_Error = abs((float)((arc.rab - get(constants.r0,j))/get(constants.r0,j)));
        ArcAngle_Error =  abs((float)((arc.angle() - get(constants.angle0,j))/get(constants.angle0,j)));
        ArcDihedral_Error = abs((float)((arc.dihedral() - get(constants.inner_dih0,j))/get(constants.inner_dih0,j)));
        ...
        reinterpret_cast<half4*>(sdata)[threadIdx.x*4 + j] = arc.gradient(constants);
    }
