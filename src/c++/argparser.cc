...
void parseArguments(int argc, char** argv, CmdArgs& args) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        std::cout << "Usage: " << argv[0] << " [-natoms <size_t>] [-nruns <size_t>] [-nwarmup <size_t>] [-nisomers <size_t>] [-nlanczos <size_t>] [-device_type <string>] [-output_file <string>] [-use_double_precision]" << std::endl;
        std::cout << "Defaults: natoms = 20, nruns = 1, nwarmup = 1, nisomers = 1, nlanczos = 50, device_type = gpu, output_file = \"output.txt\", use_double_precision = false" << std::endl;
    }
}