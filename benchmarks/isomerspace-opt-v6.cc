...
ofstream out_file("IsomerspaceOpt_V6_" + to_string(N_end) + ".txt");
  int Nd = 1;
  ...
  IsomerBatch<GPU> B0s[Nd] = {IsomerBatch<GPU>(N,batch_size,1)};
  IsomerBatch<GPU> B1s[Nd] = {IsomerBatch<GPU>(N,batch_size,1)};

  device_io::IsomerQueue Q0s[Nd] = {device_io::IsomerQueue(N,1)};
  device_io::IsomerQueue Q1s[Nd] = {device_io::IsomerQueue(N,1)};
  device_io::IsomerQueue Q2s[Nd] = {device_io::IsomerQueue(N,1)};

  for (int i = 0; i < Nd; i++) {Q0s[i].resize(batch_size*4); Q1s[i].resize(batch_size*4); Q2s[i].resize(batch_size*4);}

  LaunchCtx gen_ctxs[Nd] = {LaunchCtx(1)};
  LaunchCtx opt_ctxs[Nd] = {LaunchCtx(1)};
  ...
  auto n0 = 0;
    while(num_finished < n_fullerenes){
      ...
      auto handle = std::async(std::launch::async, generate_isomers, Nd*batch_size);
      auto T2 = steady_clock::now();
      auto loop1_condition = [&]()->bool{bool val = true; for(int i = 0; i < Nd; i++) val &= (Q1s[i].get_size() > B1s[i].capacity()); return val; };
      while(loop1_condition()){
          opt_routine();
      }
      auto T3 = steady_clock::now(); Topt += T3-T2;
      handle.wait();
      auto T4 = steady_clock::now(); Tgen += T4-T3;
      auto loop2_condition = [&]()->bool{bool val = (I==n_fullerenes && num_finished < n_fullerenes); for(int i = 0; i < Nd; i++) val &= (Q0s[i].get_size() == 0); return val; };
      while(loop2_condition()){
          opt_routine();
      }
    ...
    isomerspace_forcefield::optimize<PEDERSEN>(B1s[0], 0, 5*N, opt_ctxs[0], LaunchPolicy::ASYNC);
