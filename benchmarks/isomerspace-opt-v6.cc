...
ofstream out_file("IsomerspaceOpt_V6_" + to_string(N_end) + ".txt");
  ...
  IsomerBatch<DEVICE_BUFFER> B0s[Nd] = {IsomerBatch<DEVICE_BUFFER>(N,batch_size,1)};
  IsomerBatch<DEVICE_BUFFER> B1s[Nd] = {IsomerBatch<DEVICE_BUFFER>(N,batch_size,1)};

  cuda_io::IsomerQueue Q0s[Nd] = {cuda_io::IsomerQueue(N,1)};
  cuda_io::IsomerQueue Q1s[Nd] = {cuda_io::IsomerQueue(N,1)};
  cuda_io::IsomerQueue Q2s[Nd] = {cuda_io::IsomerQueue(N,1)};
