...
int main(int argc, char** argv){
    ...
    {   
        ...
        auto finished_isomers = 0;
            ...
            finished_isomers = 0;
            IsomerBatch HBs[Nd] = {IsomerBatch(N,mini_batch,CPU), IsomerBatch(N, mini_batch, CPU)};
            IsomerBatch DBs[Nd] = {IsomerBatch(N,batch_size,GPU,0), IsomerBatch(N, batch_size, GPU,1)};
            IsomerBatch TempBatch[Nd] = {IsomerBatch(N,mini_batch,GPU,0), IsomerBatch(N, mini_batch,GPU,1)};
            #if VALIDATION
                IsomerBatch ValBatch[Nd] = {IsomerBatch(N,batch_size,CPU), IsomerBatch(N, batch_size, CPU)};
                IsomerBatch TestBatch[Nd] = {IsomerBatch(N,batch_size,CPU), IsomerBatch(N, batch_size, CPU)};
                IsomerBatch TBs[Nd] = {IsomerBatch(N,mini_batch,CPU), IsomerBatch(N, mini_batch, CPU)};
            #endif
            ...
            for (size_t l = 0; l < N_runs + 1; l++){
                ...
                if (dual_version != 2) {for (size_t i = 0; i < Ngpu; i++) ctxs[i].start_timer();}
                auto T0 = steady_clock::now();
                switch (dual_version)
                {
                case 0:
                    for (size_t i = 0; i < Ngpu; i++) isomerspace_dual::dualize(DBs[i], ctxs[i], LaunchPolicy::ASYNC);
                    break;
                case 1:
                    for (size_t i = 0; i < Ngpu; i++) isomerspace_dual::dualize_2(DBs[i], ctxs[i], LaunchPolicy::ASYNC);
                    break;
                case 2:
                    for (size_t i = 0; i < Ngpu; i++) isomerspace_dual::dualize_3(Q_test[i].host_batch);
                    break;
                case 3:
                    for (size_t i = 0; i < Ngpu; i++) isomerspace_dual::dualize_4(Q_test[i].host_batch);
                    break;
                default:
                    break;
                }
                std::vector<nanoseconds> times(Ngpu);
                if (dual_version != 2) {for (size_t i = 0; i < Ngpu; i++) times[i] = ctxs[i].stop_timer();}
        ...
        std::cout << N << "  Dual: " << std::fixed << std::setprecision(2) << mean(T_duals)/float(batch_size*Nd) << " +- " << sdev(T_duals)/float(batch_size*Nd) <<  " Timing Diff: " << mean(T_timing_diff)/float(batch_size*Nd) << " +- " << sdev(T_timing_diff)/float(batch_size*Nd) << std::endl;
        ...
        T_duals.pop_back();
        out_file << N << ", "<< batch_size*Nd << ", " << long(mean(T_duals)) << std::endl;
        out_std << N << ", "<< batch_size*Nd << ", "  << long(sdev(T_duals)) << std::endl;
     }
