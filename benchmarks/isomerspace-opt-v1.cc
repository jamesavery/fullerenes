        ...
        for (int i = 0; i < sample_size; ++i){
            more_to_generate &= BuckyGen::next_fullerene(Q,G);
                ...
                PlanarGraph pG = G.dual_graph();
                    auto T1 = high_resolution_clock::now(); T_seq += T1 - Tstart; 
                for(int j = 0; j < N; j++){
                    for (int k = 0; k < 3; k++){
