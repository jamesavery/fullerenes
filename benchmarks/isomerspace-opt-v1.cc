...
int main(int argc, char** argv){
    ...
    {   
        ...
        for (int l = 0; l < N_runs; l++){
            device_io::IsomerQueue isomer_q(N);
            device_io::IsomerQueue out_queue(N);
            ...
            auto T1 = high_resolution_clock::now(); T_io[l] += T1 - T0;
                gpu_kernels::isomerspace_forcefield::optimize<PEDERSEN>(batch0,N*0.5,N*5);
            auto T2 = high_resolution_clock::now(); T_par[l] += T2 - T1;
                out_queue.push_done(batch0);
