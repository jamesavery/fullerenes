...
//  cout << "entering layout_pot" << endl;
  ...
  assert(k_angle.size() == 2*graph.edge_set.size()); // only for cubic graphs
...
//
  ...
  for(; e!=ee; e++){
    ...
    average_edge_length += coord2d(ax-bx,ay-by).norm();
  }
  average_edge_length /= graph.edge_set.size();
  double zero_value = .7*average_edge_length;
  //double zero_value = .4;
//  cout << "average length: " << average_edge_length << endl;
...
//  //  V = k (r - r0)**2
  ...
  for(int i=0; e!=ee; e++){
    ...
    if (it1 != graph.outer_face.end() && it2 != graph.outer_face.end()) continue; // edge is part of outer face
    const double ax = gsl_vector_get(coordinates, 2 * e->first);
    const double ay = gsl_vector_get(coordinates, 2 * e->first +1);
    const double bx = gsl_vector_get(coordinates, 2 * e->second);
    const double by = gsl_vector_get(coordinates, 2 * e->second +1);
    potential_energy += 0.5 * k_dist[i] * pow(coord2d(ax-bx,ay-by).norm() - zero_value, 2);
  }
...
//  cout << "area of outer polygon: " << A_tot << endl;
  const double A_av = abs(A_tot)/(graph.N/2+1); // (excluding the outer face)
...
//  cout << "(ought to be all zero): " << derivatives << endl;
  ...
  average_edge_length /= graph.edge_set.size();
  double zero_value = 0.7*average_edge_length;
  //double zero_value = 0.4;
//  cout << "average length: " << average_edge_length << endl;
...
//    cout << "ax " << ax << " ay " << ay << " az " << az << " bx " << bx << " by " << by << " bz " << bz << endl;
    derivatives[e->first]  += coord2d::dnorm(coord2d(ax-bx,ay-by)) * k_dist[i] * (coord2d(ax-bx,ay-by).norm() - zero_value);
    derivatives[e->second] -= coord2d::dnorm(coord2d(ax-bx,ay-by)) * k_dist[i] * (coord2d(ax-bx,ay-by).norm() - zero_value);
...
bool PlanarGraph::optimize_layout(const double k_dist_inp, const double k_angle_inp, const double k_area_inp)
{
//  cout << "entering opt layout" << endl;
  ...
  // init values
  vector<double> zero_values_dist(edge_set.size(), 1.4); // FIXME choose helpful values
  vector<double> k_dist(edge_set.size(), k_dist_inp);
  vector<double> k_angle(3 * N, k_angle_inp); // FIXME possibly change later
  vector<double> k_area(N/2 + 2, k_area_inp); // FIXME possibly change later
