...
int main(int ac, char **argv)
{
  ...
  //size_t batch_size = isomerspace_forcefield::optimal_batch_size(N,0)*16;
  ...
  CuArray<device_real_t> volume_result[Nd]       = {{batch_size,0}, {batch_size,0}};
  CuArray<device_real_t> surface_area_result[Nd] = {{batch_size,0}, {batch_size,0}};
  CuArray<device_real_t> eccentricity_result[Nd] = {{batch_size,0}, {batch_size,0}};
  ...
  LaunchCtx prop_ctxs[Nd] = {LaunchCtx(0),LaunchCtx(1)};

  BuckyGen::buckygen_queue BuckyQ = BuckyGen::start(N,IPR,only_nontrivial);  
  ProgressBar progress_bar = ProgressBar('#',30);
  
  typedef enum {GENERATE, OPTIMIZE, PROPERTIES, N_STAGES} stages; 
  
  int I=0;			// Global isomer number at start of batch
  array<int, N_STAGES> num_finished = {{0,0,0}};
  array<bool,N_STAGES> done = {{false,false,false}};
  int
      num_converged = 0,
      num_failed =0;
  Graph G;
  ...
  auto
    ...
    Tprop1  = steady_clock::now()-T0,
    Tprop2  = steady_clock::now()-T0,
    Tprop3  = steady_clock::now()-T0;

  auto launch_policy = LaunchPolicy::SYNC;
  ...
  auto generate_isomers = [&](int M){
    ...
    for (int i = 0; i < M; i++){
        if (more_work_to_do){
              ...
              more_work_to_do &= BuckyGen::next_fullerene(BuckyQ, G); 
              if(!more_work_to_do) break;
              Q0s[I%Nd].insert(G,I, gen_ctxs[I%Nd], launch_policy); // Host insert 1 graph
            I++;
        }
    }
    gen_ctxs[0].wait(); gen_ctxs[1].wait(); 
    done[GENERATE] = !more_work_to_do;
    return more_work_to_do;
  };

  auto opt_routine = [&](){
    size_t qlen[Nd] = {0,0};
    for(int d=0;d<Nd;d++) qlen[d] = Q2s[d].get_size();    
    for (int i = 0; i < Nd; i++){
      Q1s[i].refill_batch(B1s[i], opt_ctxs[i], launch_policy);
      std::cout 
        << "Q1 = " << Q1s[i] << "\n"
        << "B1 = " << B1s[i] << "\n\n";
    ...
    done[OPTIMIZE] = num_finished[OPTIMIZE] >= n_fullerenes;
    printf("opt_routine: done=%d, num_finished=%d, n_fullerenes=%d\n",done[OPTIMIZE], num_finished[OPTIMIZE],n_fullerenes);
  };
  ...
  auto prop_routine = [&](){
    ...
    for(int d=0;d<Nd;d++) qlen[d] = Q2s[d].get_size();
    for(int d=0;d<Nd;d++){
      Q2s[d].refill_batch(B2s[d],gen_ctxs[d],launch_policy);
      isomerspace_properties::transform_coordinates(B2s[d],prop_ctxs[d],launch_policy);
      isomerspace_properties::volume_divergences(B2s[d],  volume_result[d], prop_ctxs[d], launch_policy);
      isomerspace_properties::surface_areas(B2s[d], surface_area_result[d], prop_ctxs[d], launch_policy);
      isomerspace_properties::eccentricities(B2s[d],eccentricity_result[d], prop_ctxs[d], launch_policy);        
    }
    for(int d=0;d<Nd;d++){
      prop_ctxs[d].wait();
      num_finished[PROPERTIES] += qlen[d]-Q2s[d].get_size();
    } 
    done[PROPERTIES] = num_finished[PROPERTIES] >= n_fullerenes;
  };

  auto last_bit = [&](bool previous_step_done, const IsomerQueue *Q) { // TODO Nd everywhere
    return I == n_fullerenes && previous_step_done && Q[0].get_size() == 0 && Q[1].get_size() == 0;
  };

  auto queue_has_enough = [&](const IsomerQueue *Q,const IsomerBatch *B){
    return Q[0].get_size() > B[0].capacity() && Q[1].get_size() > B[1].capacity();
  };
  ...
  while(!done[PROPERTIES]){
      auto T0 = steady_clock::now();
      for (int i = 0; i < Nd; i++){
          if(Q0s[i].get_size() > 0){
            Q0s[i].refill_batch(B0s[i], gen_ctxs[i], launch_policy);
            isomerspace_dual::dualise(B0s[i], gen_ctxs[i], launch_policy);
            isomerspace_tutte::tutte_layout(B0s[i], N*10, gen_ctxs[i], launch_policy);
            isomerspace_X0::zero_order_geometry(B0s[i], 4.0f, gen_ctxs[i], launch_policy);
            Q1s[i].insert(B0s[i], gen_ctxs[i], launch_policy); // GPU insert batch
          }
      }
      for(int j = 0; j < Nd; j++){
        gen_ctxs[j].wait();
      }
      printf("Generated I=%d graphs. %s done generating.\n",I, done[GENERATE]? "I am":"Not");
      auto T1 = steady_clock::now(); Tinit_geom += T1-T0;
      auto handle = std::async(std::launch::async, generate_isomers, 2*batch_size);
      handle.wait();/* TODO: tilbage */
      auto T2 = steady_clock::now();
      while(queue_has_enough(Q1s, B1s)){
          opt_routine();
      }
      ...
      if(last_bit(done[GENERATE],Q0s)){
          printf("Last bit for opt_routine(); num_finished=%d.\n",num_finished[OPTIMIZE]);
          while(!done[OPTIMIZE]) opt_routine();
      }
      printf("Optimized %d graphs. %s done optimizing.\n",
            num_finished[OPTIMIZE],done[OPTIMIZE]?"I am":"Not");
      auto T5 = steady_clock::now(); Topt += T5-T4;
      // 
      if(queue_has_enough(Q2s,B2s))      prop_routine();
      if(last_bit(done[GENERATE],Q0s) && last_bit(done[OPTIMIZE],Q1s)) prop_routine();
      
      auto T6 = steady_clock::now(); Tprop1 += T6-T5;
      printf("Calculated properties for %d graphs. %s done with propertizing.\n",
          num_finished[PROPERTIES], done[PROPERTIES]?"I am":"Not");

      if(loop_iters % 3 == 0 || done[PROPERTIES]){
  ...
  std::cout << "volumes[0] = " << volume_result[0] << ";\n";
  std::cout << "volumes[1] = " << volume_result[1] << ";\n";
  std::cout << "eccentricities[0] = " << eccentricity_result[0] << ";\n";
  std::cout << "eccentricities[1] = " << eccentricity_result[1] << ";\n";
