...
typedef enum
{
  ...
  INERTIA,
  NUM_RESULTS
} results_t; // Results on which to do statistics
...
struct isomer_candidate
{
  ...
  isomer_candidate(double value, int id, array<float, NUM_RESULTS> &results, Fullerene<real_t, device_node_t> fullerene) : value(value), id(id), results(results), cubic_neighbours(3 * fullerene.N_), X(3 * fullerene.N_) /*, H(90*N), Hcol(90*N)*/
  {                                                                                                                                                                                                                        // Det tager 50% ekstra tid at gemme Hessian'en
...
int main(int ac, char **argv)
{
  ...
  auto geo_routine = [&]()
  {
    ...
    std::for_each(Q1s.begin(), Q1s.end(), [&](auto &Q)
                  { qsize_geo.push_back(Q.size()); });
    for (int d = 0; d < Nd; d++)
    {
      if (Q0s[d].size() > 0)
      {
        ...
        auto &ctx = gen_ctxs[d];
        real_t scalerad = 4;
        QueueUtil::push(ctx, B, Q0s[d], ConditionFunctor(StatusEnum::EMPTY));
        dualize(ctx, B, policy);
        tutte(ctx, B, policy);
        spherical_projection(ctx, B, policy);
        QueueUtil::push(ctx, Q1s[d], B, ConditionFunctor(StatusEnum::NOT_CONVERGED), StatusEnum::EMPTY); // Push all to next stage
      }
    }
    for (int d = 0; d < Nd; d++)
    {
  ...
  auto opt_routine = [&](bool final = false)
  {
    ...
    std::for_each(Q2s.begin(), Q2s.end(), [&](auto &Q)
                  { qsize_opt.push_back(Q.size()); });
    ...
    /* if (final)
    {
      for (int d = 0; d < Nd; d++)
      {
        ...
        for (size_t di = 0; di < Bs[PROP][d].capacity(); di++)
          n_states[size_t(Bs[PROP][d].m_.flags_[di])]++;
      }
    } */
    return true;
  };
  ...
  auto prop_routine = [&]()
  {
    ...
    for (int d = 0; d < Nd; d++)
    {
      ...
      spectrum_ends(ctx, B, policy, hessians[d], hessian_cols[d], size_t(50), intermediate_spectrum_ends[d], eigenvectorsss);
  ...
  auto stat_routine = [&]()
  {
    ...
    for (int d = 0; d < Nd; d++)
    {
      ...
      for (int di = 0; di < num_finished_this_round[PROP][d]; di++, i++)
      {
        ...
        if (status.is_set(StatusEnum::CONVERGED_3D))
        {
          ...
          for (int r = 0; r < NUM_RESULTS; r++)
            if (result_sizes[r] == 1)
            {
              ...
              if (isfinite(v))
              {
                isomer_candidate C(0, id, R, Bs[PROP][d][di]); //No need to construct C if v is nan
                C.value = v;
                result_min[r].insert(C);
                C.value = -v;
                result_max[r].insert(C);
                means[r] += v;
              }
        ...
        {
          if (status.is_set(StatusEnum::FAILED_3D))
            opt_failed.push_back(id);
          if (status.is_set(StatusEnum::NOT_CONVERGED))
            opt_not_converged.push_back(id);
        }
      }
    }
    ...
    if (num_finished[STAT] >= n_fullerenes)
      stage_finished[STAT] = true; // TODO: We're overcounting somehow, getting num_finished > n_fullerenes. DEBUG!
  ...
  while (!stage_finished[PROP])
  {
    ...
    if (final_round(PROP))
    {
      fprintf(stderr, "final prop start\n");
      prop_routine();
      fprintf(stderr, "final prop end\n");
    }
    if (final_round(STAT))
    {
      fprintf(stderr, "final stat start\n");
      stat_routine();
      fprintf(stderr, "final stat end\n");
    }
  ...
  auto host_points = [](const vector<real_t> &X)
  {
    ...
    for (node_t u = 0; u < N; u++)
      points[u] = {X[3 * u], X[3 * u + 1], X[3 * u + 2]};
    return points;
  };
    ...
    if (STORE_HESSIAN)
    {
      for (int d = 0; d < Nd; d++)
      {
        ...
        if (STORE_EIGENSYSTEM)
        {
          ...
          eigen(ctx, B, policy, hessians[d], hessian_cols[d], N*3 - 6, lams[d], Q[d]);
        }
        ctx.wait();
      }
    }
  ...
  }
  {
    ...
    if (!f) {
      std::cerr << "Error opening file for writing: " << output_dir + "/result_bounds.float64" << std::endl;
      return -1;
    }
  ...
  for (int r = 0; r < NUM_RESULTS; r++)
    if (result_sizes[r] == 1)
    {
      ...
      for (size_t i = 0; i < n_best_candidates; i++)
      {
        ...
        auto Pmin = (Polyhedron)final_batch[d_min][di_min]; //.get_isomer(di_min).value();
        auto Pmax = (Polyhedron)final_batch[d_max][di_max]; //.get_isomer(di_max).value();
        ...
        { // TODO: binary_write til funktion, som ogsaa tjekker for fejl + perror()'er
          ...
          //	fwrite(&final_batch[d_min].cubic_neighbours+di_min*3*N, sizeof(device_node_t), 3*N,f);
          ...
          fwrite(final_batch[d_min].d_.X_cubic_.data() + di_min * 3 * N, sizeof(real_t), 3 * N, f);
          fclose(f);

          f = fopen((max_basename + "-X.float32").c_str(), "wb");
          fwrite(final_batch[d_max].d_.X_cubic_.data() + di_max * 3 * N, sizeof(real_t), 3 * N, f);
          fclose(f);
