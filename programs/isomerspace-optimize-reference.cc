...
int main(int ac, char **argv)
{
  ...
  auto batch_size = gpu_kernels::isomerspace_forcefield::optimal_batch_size(N,0)*4;
  ...
  while (more_to_generate)
  {
    ...
    while (Q0.get_size() > 0){
      ...
      auto T3 = system_clock::now(); Ttutte += T3-T2;
    ...
    while (Q1.get_size() > batch2.isomer_capacity)
    {
      ...
      gpu_kernels::isomerspace_forcefield::optimize_batch(batch2,N*1,N*50,ctx0,LaunchPolicy::ASYNC);
      auto T1 = system_clock::now();
    ...
    if (!more_to_generate ){
      ...
      while(Q1.get_size() > 0){
        ...
        gpu_kernels::isomerspace_forcefield::optimize_batch(batch2,N*5,N*5);
    ...
    if (num_finished > 160000) break;
  }
  auto Ttot = system_clock::now() - T0;
  Topt   = (1ms*int(gpu_kernels::isomerspace_forcefield::time_spent()));
  Ttutte = (1ms*int(gpu_kernels::isomerspace_tutte::time_spent()));
  TX0    = (1ms*int(gpu_kernels::isomerspace_X0::time_spent()));
