...
int main(int ac, char **argv)
{
  ...
  IsomerBatch batch0(N,1088,DEVICE_BUFFER);
  IsomerBatch batch1(N,1088,DEVICE_BUFFER);
  IsomerBatch batch2(N,1088,DEVICE_BUFFER);
  
  cuda_io::BatchQueue Q0 = cuda_io::BatchQueue(N);
  cuda_io::BatchQueue Q1 = cuda_io::BatchQueue(N);
  cuda_io::BatchQueue Q2 = cuda_io::BatchQueue(N);
  ...
  while (I != num_fullerenes.find(N)->second )
  {
  ...
  gpu_kernels::isomerspace_X0::zero_order_geometry(batch1, 4);

  Q2.insert(batch1);
  Q2.refill_batch(batch2);
  gpu_kernels::isomerspace_forcefield::optimize_batch(batch2, N, N*10);


  cuda_io::output_to_queue(output_queue,batch2,false);
  std::cout << "size: " << Q2.get_size() << std::endl;
